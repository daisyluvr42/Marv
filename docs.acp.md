...
This document describes how the Marv ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`marv acp` exposes an ACP agent over stdio and forwards prompts to a running
Marv Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
marv acp --session agent:design:main
marv acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Marv ACP” to start a thread.

## Execution Model

- ACP client spawns `marv acp` and speaks ACP messages over stdio.
...
```bash
marv acp --session agent:main:main
marv acp --session-label "support inbox"
marv acp --reset-session
```
...
`marv acp` resolves the Gateway URL and auth from CLI flags or config:
