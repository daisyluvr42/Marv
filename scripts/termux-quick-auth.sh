...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/marv/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Marv Re-Auth" -c "After getting key, run: ssh $SERVER '~/marv/scripts/mobile-reauth.sh'" --id marv-auth
        ;;
    *)
