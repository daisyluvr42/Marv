...
BASE_IMAGE="${BASE_IMAGE:-marv-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-marv-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=marv.sandbox=1)
NOTE
