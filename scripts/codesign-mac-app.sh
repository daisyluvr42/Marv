...
APP_BUNDLE="${1:-dist/Marv.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t marv-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t marv-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t marv-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Marv" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Marv" "$APP_ENTITLEMENTS"
fi
