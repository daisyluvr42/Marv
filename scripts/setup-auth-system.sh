#!/bin/bash
# Setup Marv Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'marv-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Marv message: Send warning via Marv itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/marv-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/marv-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/marv-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now marv-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status marv-auth-monitor.timer"
echo ""
echo "Quick commands:"
