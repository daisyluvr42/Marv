...
const shouldBuild = (deps) => {
  if (deps.env.MARV_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MARV_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[marv] ${message}\n`);
};

const runMarv = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["marv.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
