...
ZIP=${1:?"Usage: $0 Marv-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/marv/marv/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Marv-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"/download/v${VERSION}/"}
