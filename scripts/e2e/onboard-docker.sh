...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    MARV_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    MARV_ENTRY="dist/index.js"
	  else
  ...
  export PATH="/tmp/marv-bin:$PATH"
  mkdir -p /tmp/marv-bin
  cat > /tmp/marv-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$MARV_ENTRY" gateway --port 4242 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  wait_for_gateway() {
    for _ in $(seq 1 20); do
      if node --input-type=module -e "
        ...
        const socket = net.createConnection({ host: '127.0.0.1', port: 4242 });
      ...
      if [ -f /tmp/gateway-e2e.log ] && grep -E -q "listening on ws://[^ ]+:4242" /tmp/gateway-e2e.log; then
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/marv-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/marv-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$MARV_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/marv-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.marv/workspace"
    config_path="$HOME/.marv/marv.json"
    sessions_dir="$HOME/.marv/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$MARV_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
	      --remote-url ws://gateway.local:4242 \
...
if (cfg?.gateway?.remote?.url !== "ws://gateway.local:4242") {
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.marv/marv.json" <<'"'"'JSON'"'"'
{
  "agents": { "defaults": { "workspace": "/root/old" } },
  "gateway": {
    "mode": "remote",
    "remote": { "url": "ws://old.example:4242", "token": "old-token" }
  }
}
JSON
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$MARV_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$MARV_ENTRY\" configure --section skills" send_skills_flow
