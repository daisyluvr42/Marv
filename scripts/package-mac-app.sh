...
# Build and bundle Marv into a minimal .app we can open.
# Outputs to dist/Marv.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Marv.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Marv"
BUNDLE_ID="${BUNDLE_ID:-ai.marv.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/marv/marv/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Marv/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MarvBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MarvGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Marv"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Marv"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Marv"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Marv" 2>/dev/null || true
...
echo "📦 Copying MarvKit resources"
MARVKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MarvKit_MarvKit.bundle"
if [ -d "$MARVKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MarvKit_MarvKit.bundle"
  cp -R "$MARVKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MarvKit_MarvKit.bundle"
else
  echo "WARN: MarvKit resource bundle not found at $MARVKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Marv"
killall -q Marv 2>/dev/null || true
