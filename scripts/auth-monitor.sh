...
#
# Suggested cron: */30 * * * * /home/admin/marv/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Marv notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., marv-alerts)
...
STATE_FILE="$HOME/.marv/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Marv if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use marv
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Marv to $NOTIFY_PHONE..."
            marv send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Marv is down. Run: ssh l36 '~/marv/scripts/mobile-reauth.sh'" "urgent"
    exit 1
