 ...
 * `marv update`. Use it to verify the completion installation flow
...
\`marv update\`, \`marv doctor\`, and \`marv onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Marv Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`marv completion --install\` later to enable.`));
    return;
  }
