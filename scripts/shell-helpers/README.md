...
Inspired by Simon Willison's [Running Marv in Docker](https://til.simonwillison.net/llms/marv-docker).
...
On first command, ClawDock auto-detects your Marv directory:

- Checks common paths (`~/marv`, `~/workspace/marv`, etc.)
...
| `clawdock-cli <command>`  | Run Marv CLI commands                          |
...
| `clawdock-cd`        | Jump to the Marv project directory        |
| `clawdock-config`    | Open the Marv config directory            |
...
```bash
marv channels login --channel whatsapp --verbose
```
...
2. Configure it in the Marv config
...
- Bash or Zsh shell
- Marv project (from `docker-setup.sh`)
