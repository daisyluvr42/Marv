#!/usr/bin/env bash
# ClawDock - Docker helpers for Marv
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/marv"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MARV_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d marv-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart marv-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f marv-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec marv-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm marv-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build marv-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MARV_GATEWAY_TOKEN=$token" marv-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" marv-gateway \
    bash -c './marv.mjs config set gateway.remote.token "$TOKEN" && ./marv.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec marv-gateway \
    bash -c "./marv.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart marv-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm marv-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec marv-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🤖 ClawDock - Docker Helpers for Marv${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (marv alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to marv project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.marv)${_CLR_RESET}"
