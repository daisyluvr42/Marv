...
- Gateway/CLI: add paired-device hygiene flows with `device.pair.remove`, plus `marv devices remove` and guarded `marv devices clear --yes [--pending]` commands for removing paired entries and optionally rejecting pending requests. (#20057) Thanks @mbelinky.
...
- MarvKit/Protocol: preserve JSON boolean literals (`true`/`false`) when bridging through `AnyCodable` so Apple client RPC params no longer re-encode booleans as `1`/`0`. Thanks @mbelinky.
...
- Browser/Relay: reuse an already-running extension relay when the relay port is occupied by another Marv process, while still failing on non-relay port collisions to avoid masking unrelated listeners. (#20035) Thanks @mbelinky.
...
- iOS/Signing: restore local auto-selected signing-team overrides during iOS project generation by wiring `.local-signing.xcconfig` into the active signing config and emitting `MARV_DEVELOPMENT_TEAM` in local signing setup. (#19993) Thanks @ngutman.
...
- Cron/CLI: add deterministic default stagger for recurring top-of-hour cron schedules (including 6-field seconds cron), auto-migrate existing jobs to persisted `schedule.staggerMs`, and add `marv cron add/edit --stagger <duration>` plus `--exact` overrides for per-job timing control.
...
- Telegram: add poll sending via `marv message poll` (duration seconds, silent delivery, anonymity controls). (#16209) Thanks @robbyczgw-cla.
- Slack/Discord: add `dmPolicy` + `allowFrom` config aliases for DM access control; legacy `dm.policy` + `dm.allowFrom` keys remain supported and `marv doctor --fix` can migrate them.
...
- CLI/Plugins: ensure `marv message send` exits after successful delivery across plugin-backed channels so one-shot sends do not hang. (#16491) Thanks @yinghaosang.
- CLI/Plugins: run registered plugin `gateway_stop` hooks before `marv message` exits (success and failure paths), so plugin-backed channels can clean up one-shot CLI resources. (#16580) Thanks @gumadeiras.
...
- TUI/Gateway: resolve local gateway target URL from `gateway.bind` mode (tailnet/lan) instead of hardcoded localhost so `marv tui` connects when gateway is non-loopback. (#16299) Thanks @cortexuvula.
- TUI: honor explicit `--session <key>` in `marv tui` even when `session.scope` is `global`, so named sessions no longer collapse into shared global history. (#16575) Thanks @cinqu.
...
- Memory/QMD: make `marv memory index` verify and print the active QMD index file path/size, and fail when QMD leaves a missing or zero-byte index artifact after an update. (#16775) Thanks @Shunamxiao.
...
- Media/Security: allow local media reads from Marv state `workspace/` and `sandboxes/` roots by default so generated workspace media can be delivered without unsafe global path bypasses. (#15541) Thanks @lanceji.
...
- Security/Hooks: restrict hook transform modules to `~/.marv/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Telegram/Security: require numeric Telegram sender IDs for allowlist authorization (reject `@username` principals), auto-resolve `@username` to IDs in `marv doctor --fix` (when possible), and warn in `marv security audit` when legacy configs contain usernames. Thanks @vincentkoc.
...
- macOS: hard-limit unkeyed `marv://agent` deep links and ignore `deliver` / `to` / `channel` unless a valid unattended key is provided. Thanks @Cillian-Collins.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (marv user, image, launch script, systemd quadlet), `run-marv-podman.sh launch` / `launch setup`; systemd Quadlet unit for marv user service; docs for rootless container, marv user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- Config/State: removed legacy `.moltbot` auto-detection/migration and `moltbot.json` config candidates. If you still have state/config under `~/.moltbot`, move it to `~/.marv` (recommended) or set `MARV_STATE_DIR` / `MARV_CONFIG_PATH` explicitly.

### Fixes

- Gateway/Auth: add trusted-proxy mode hardening follow-ups by keeping `MARV_GATEWAY_*` env compatibility, auto-normalizing invalid setup combinations in interactive `gateway configure` (trusted-proxy forces `bind=lan` and disables Tailscale serve/funnel), and suppressing shared-secret/rate-limit audit findings that do not apply to trusted-proxy deployments. (#15940) Thanks @nickytonline.
...
- OpenAI Codex/Auth: bridge Marv OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- CLI/Completion: route plugin-load logs to stderr and write generated completion scripts directly to stdout to avoid `source <(marv completion ...)` corruption. (#15481) Thanks @arosstale.
...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to Marv temp roots and reject traversal/escape paths.
...
- Security/Browser: confine `POST /hooks/file-chooser` upload paths to an Marv temp uploads root and reject traversal/escape paths. Thanks @1seal.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `marv config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- Config: preserve `${VAR}` env references when writing config files so `marv config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- CLI/Plugins: add `marv plugins uninstall <id>` with `--dry-run`, `--force`, and `--keep-files` options, including safe uninstall path handling and plugin uninstall docs. (#5985) Thanks @JustasMonkev.
- CLI: add `marv logs --local-time` to display log timestamps in local timezone. (#13818) Thanks @xialonglee.
...
- Logging/CLI: use local timezone timestamps for console prefixing, and include `±HH:MM` offsets when using `marv logs --local-time` to avoid ambiguity. (#14771) Thanks @0xRaini.
...
- Gateway/Control UI: resolve missing dashboard assets when `marv` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Logging/Browser: fall back to `os.tmpdir()/marv` for default log, browser trace, and browser download temp paths when `/tmp/marv` is unavailable.
...
- Feishu plugin packaging: remove `workspace:*` `marv` dependency from `extensions/feishu` and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Update/Daemon: fix post-update restart compatibility by generating `dist/cli/daemon-cli.js` with alias-aware exports from hashed daemon bundles, preventing `registerDaemonCli` import failures during `marv update`.
...
- Paths: add `MARV_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- CLI: make `marv plugins list` output scannable by hoisting source roots and shortening bundled/global/workspace plugin paths.
...
- Paths: structurally resolve `MARV_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- Memory/QMD: run boot refresh in background by default, add configurable QMD maintenance timeouts, retry QMD after fallback failures, and scope QMD queries to Marv-managed collections. (#9690, #9705, #10042) Thanks @vignesh07.
...
- State dir: honor `MARV_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- macOS: honor Nix-managed defaults suite (`ai.marv.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Update: avoid wiping prebuilt Control UI assets during dev auto-builds (`tsdown --no-clean`), run update doctor via `marv.mjs`, and auto-restore missing UI assets after doctor. (#10146) Thanks @gumadeiras.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (marv-cn).
...
- Docs: finish renaming the QMD memory docs to reference the Marv state dir.
...
- Skills: update session-logs paths from .clawdbot to .marv. (#4502)
...
- Build: align npm tar override and bin metadata so the `marv` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update Marv branding + install links.
...
- Rebrand: rename the npm package/CLI to `marv`, add a `marv` compatibility shim, and move extensions to the `@marv/*` scope.
...
- macOS: finish Marv app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Skills: update session-logs paths to use ~/.marv. (#4502) Thanks @bonald.
...
- CLI: add `marv system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d)
- CLI: add live auth probes to `marv models status` for per-profile verification. (commit 40181afde)
- CLI: restart the gateway by default after `marv update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `marv models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- CLI: flatten node service commands under `marv node` and remove `service node` docs.
- CLI: move gateway service commands under `marv gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `marv update wizard` for interactive channel selection and restart prompts.
...
- ACP: add `marv acp` for IDE integrations.
- ACP: add `marv acp client` interactive harness for debugging.
...
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `marv update`.
- Plugins: share npm plugin update logic between `marv update` and `marv plugins update`.
...
- Exec approvals: migrate approvals to `~/.marv/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events.
- Nodes: add headless node host (`marv node start`) for `system.run`/`system.which`.
...
- Build: use tsgo for dev/watch builds by default (opt out with `MARV_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `marv doctor --fix` to repair, then update plugins (`marv plugins update`) if you use any.
...
- CLI: keep `marv logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `marv memory status`.
...
- Memory: probe sqlite-vec availability in `marv memory status`.
...
- Plugins: add Zalo Personal plugin (`@marv/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh.
...
- **BREAKING:** `marv message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `marv hooks` is now `marv webhooks`; hooks live under `marv hooks`.
- **BREAKING:** `marv plugins install <path>` now copies into `~/.marv/extensions` (use `--link` to keep path-based loading).
...
- Directory: unify `marv directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `marv.hooks` manifests and `marv hooks install/update`.
...
- CLI: speed up `marv sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `marv models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@marv/msteams` via `marv plugins install @marv/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `marv-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `marv security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `marv models status`, and update docs.
- CLI: add `--json` output for `marv daemon` lifecycle/install commands.
...
- Fix: make `marv update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `marv configure`, even if no other sections are chosen.
...
- Security: expanded `marv security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `marv security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `marv browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.marv-dev`).
...
- Memory: new `marv memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.marv/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `marv dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `marv doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `marv doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `marv plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `marv` CLI when missing in local mode; install via `marv.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch marv.ai scripts and run install sh/cli with onboarding skipped.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `marv.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `marv status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `marv update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `marv reset` and `marv uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `marv gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `marv status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `marv sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `marv sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “Marv” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `marv update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/marv`. (#660)
...
- CLI: `marv message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `marv models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.marv/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `marv pairing list <provider>` + `marv pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `marv logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$MARV_STATE_DIR/.env` (`~/.marv/.env`) as a fallback after CWD `.env`.
...
- CLI: run `marv agent` via the Gateway by default; use `--local` to force embedded mode.
