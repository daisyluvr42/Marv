...
Marv extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
marv channels login --channel zalouser
...
```bash
marv message send --channel zalouser --target <threadId> --message "Hello from Marv!"
```
...
```bash
marv channels login --channel zalouser              # Login via QR
marv channels login --channel zalouser --account work
marv channels status --probe
marv channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
marv directory self --channel zalouser
marv directory peers list --channel zalouser --query "name"
marv directory groups list --channel zalouser --query "work"
marv directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
marv message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
marv message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `marv channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
marv channels login --channel zalouser --account work
marv message send --channel zalouser --account work --target <id> --message "Hello"
