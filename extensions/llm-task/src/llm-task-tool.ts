...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with Marv.
// When running from source (tests/dev), Marv internals live under src/.
...
async function loadRunEmbeddedPiAgent(): Promise<RunEmbeddedPiAgentFn> {
  ...
  try {
    const mod = await import("../../../src/agents/runner/pi-embedded-runner.js");
...
export function createLlmTaskTool(api: MarvPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via marv.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-llm-task-"));
