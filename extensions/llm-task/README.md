...
`marv.invoke --each`) without adding new Marv code per workflow.
...
This extension depends on Marv internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Marv extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.marv/extensions` as a standalone plugin directory.
