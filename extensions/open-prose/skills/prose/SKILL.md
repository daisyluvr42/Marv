...
metadata: { "marv": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Marv `sessions_spawn`
- **File I/O** == Marv `read`/`write`
- **Remote fetch** == Marv `web_fetch` (or `exec` with curl when POST is required)
