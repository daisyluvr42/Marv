...
function resolveMarvRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MARV_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MARV_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMarvRoot(), "dist", "extensionAPI.js");
