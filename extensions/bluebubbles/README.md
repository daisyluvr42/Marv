...
This directory contains the **BlueBubbles external channel plugin** for Marv.
...
- Webhook handling: `extensions/bluebubbles/src/monitor.ts` (register via `registerPluginHttpRoute`).
...
- Route into core reply pipeline via the plugin runtime (`api.runtime`) and `agentmarv/plugin-sdk` helpers.
