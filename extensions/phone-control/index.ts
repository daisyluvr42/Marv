...
function normalizeDenyList(cfg: MarvPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MarvPluginApi["config"]): string[] {
...
export default function register(api: MarvPluginApi) {
  ...
  const timerService: MarvPluginService = {
