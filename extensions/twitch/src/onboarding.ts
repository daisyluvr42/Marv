 ...
 */
function setTwitchAccount(cfg: MarvConfig, account: Partial<TwitchAccountConfig>): MarvConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: MARV_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: MarvConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var MARV_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.MARV_TWITCH_ACCESS_TOKEN?.trim();
