/**
 * Twitch channel plugin for Marv.
 *
...
import { buildChannelConfigSchema } from "agentmarv/plugin-sdk";
 ...
 * for Marv. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MarvConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MarvConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MarvConfig): boolean => {
