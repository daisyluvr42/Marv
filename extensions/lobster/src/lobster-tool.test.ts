...
import type { MarvPluginApi, MarvPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<MarvPluginApi> = {}): MarvPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<MarvPluginToolContext> = {}): MarvPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: MarvPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
