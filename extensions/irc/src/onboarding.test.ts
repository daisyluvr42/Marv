import type { RuntimeEnv, WizardPrompter } from "agentmarv/plugin-sdk";
...
describe("irc onboarding", () => {
  it("configures host and nick via onboarding prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("marv-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#marv", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#marv", "#ops"]);
  });
