...
@Suite struct DeepLinkParserTests {
    @Test func parseRejectsUnknownHost() {
        let url = URL(string: "marv://nope?message=hi")!
    ...
    @Test func parseRejectsNonMarvScheme() {
        let url = URL(string: "https://example.com/agent?message=hi")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseRejectsEmptyMessage() {
        let url = URL(string: "marv://agent?message=%20%20%0A")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseAgentLinkParsesCommonFields() {
        let url =
            URL(string: "marv://agent?message=Hello&deliver=1&sessionKey=node-test&thinking=low&timeoutSeconds=30")!
        #expect(
    ...
    @Test func parseAgentLinkParsesTargetRoutingFields() {
        let url =
            URL(
                string: "marv://agent?message=Hello%20World&deliver=1&to=%2B15551234567&channel=whatsapp&key=secret")!
        #expect(
            DeepLinkParser.parse(url) == .agent(
                .init(
                    message: "Hello World",
    ...
    @Test func parseRejectsNegativeTimeoutSeconds() {
        let url = URL(string: "marv://agent?message=Hello&timeoutSeconds=-1")!
        #expect(DeepLinkParser.parse(url) == .agent(.init(
            message: "Hello",
            sessionKey: nil,
            thinking: nil,
    ...
    @Test func parseGatewayLinkParsesCommonFields() {
        let url = URL(
            string: "marv://gateway?host=marv.local&port=18789&tls=1&token=abc&password=def")!
        #expect(
            DeepLinkParser.parse(url) == .gateway(
                .init(host: "marv.local", port: 18789, tls: true, token: "abc", password: "def")))
    }
