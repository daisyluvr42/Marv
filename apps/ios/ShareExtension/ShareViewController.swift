...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.marv.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Marv", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Marv gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "MarvShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Marv is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("marv-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
