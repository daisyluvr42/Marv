...
final class MotionService: MotionServicing {
    func activities(params: MarvMotionActivityParams) async throws -> MarvMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MarvMotionActivityEntry], Error>) in
        ...
        return MarvMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MarvPedometerParams) async throws -> MarvPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<MarvPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MarvPedometerPayload(
