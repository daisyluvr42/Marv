...
struct IOSGatewayChatTransport: MarvChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.marv", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MarvChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MarvChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MarvChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MarvChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MarvChatAttachmentPayload]) async throws -> MarvChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MarvChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MarvChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MarvGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MarvGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MarvChatEventPayload.self)
                        {
