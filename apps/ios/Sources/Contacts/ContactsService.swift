...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MarvContactsSearchParams) async throws -> MarvContactsSearchPayload {
        ...
        return MarvContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MarvContactsAddParams) async throws -> MarvContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MarvContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MarvContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MarvContactPayload {
        MarvContactPayload(
