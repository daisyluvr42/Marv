...
final class RemindersService: RemindersServicing {
    func list(params: MarvRemindersListParams) async throws -> MarvRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MarvReminderPayload], Error>) in
        ...
        return MarvRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MarvRemindersAddParams) async throws -> MarvRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MarvReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MarvRemindersAddPayload(reminder: payload)
    }
