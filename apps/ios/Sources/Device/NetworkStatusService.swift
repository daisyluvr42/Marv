...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> MarvNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> MarvNetworkStatusPayload {
        let status: MarvNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> MarvNetworkStatusPayload {
        MarvNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
