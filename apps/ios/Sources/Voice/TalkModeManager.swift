...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MARV_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MarvTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MarvTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MarvTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MarvTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MarvTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MarvTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> MarvTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MarvTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MarvTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: MarvAgentEventPayload.self) else {
                continue
            }
