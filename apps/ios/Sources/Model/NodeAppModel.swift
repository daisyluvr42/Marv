...
final class NodeAppModel {
    private let deepLinkLogger = Logger(subsystem: "ai.marv.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.marv.ios", category: "PushWake")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = MarvCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = MarvCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = MarvCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = MarvCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = MarvCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: MarvLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(MarvGatewayHealthOK.self, from: data) else {
                        return false
                    }
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for marv:// links.)
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: MarvNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: MarvNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarvNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(MarvLocationGetParams.self, from: req.paramsJSON)) ??
            MarvLocationGetParams()
        ...
        let payload = MarvLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(MarvCanvasPresentParams.self, from: req.paramsJSON)) ??
                MarvCanvasPresentParams()
        ...
        case MarvCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MarvCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(MarvCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MarvCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(MarvCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarvCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(MarvCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case MarvCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.marvA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing marvA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvCanvasA2UICommand.push.rawValue, MarvCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [MarvKit.AnyCodable]
            if command == MarvCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(MarvCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try MarvCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(MarvCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try MarvCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.marvA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarvCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(MarvCameraSnapParams.self, from: req.paramsJSON)) ??
                MarvCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarvCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(MarvCameraClipParams.self, from: req.paramsJSON)) ??
                MarvCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(MarvScreenRecordParams.self, from: req.paramsJSON)) ??
            MarvScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(MarvSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarvNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarvNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarvNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(MarvChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarvNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = MarvChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(MarvPhotosLatestParams.self, from: req.paramsJSON)) ??
            MarvPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(MarvContactsSearchParams.self, from: req.paramsJSON)) ??
                MarvContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvContactsCommand.add.rawValue:
            let params = try Self.decodeParams(MarvContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(MarvCalendarEventsParams.self, from: req.paramsJSON)) ??
                MarvCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(MarvCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(MarvRemindersListParams.self, from: req.paramsJSON)) ??
                MarvRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(MarvRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(MarvMotionActivityParams.self, from: req.paramsJSON)) ??
                MarvMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(MarvPedometerParams.self, from: req.paramsJSON)) ??
                MarvPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([MarvLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            MarvCameraCommand.list.rawValue,
            MarvCameraCommand.snap.rawValue,
            MarvCameraCommand.clip.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleCameraInvoke(req)
        }

        register([MarvScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([MarvSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([MarvChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleChatPushInvoke(req)
        }

        register([
            MarvDeviceCommand.status.rawValue,
            MarvDeviceCommand.info.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            MarvWatchCommand.status.rawValue,
            MarvWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([MarvPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handlePhotosInvoke(req)
        }

        register([
            MarvContactsCommand.search.rawValue,
            MarvContactsCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            MarvCalendarCommand.events.rawValue,
            MarvCalendarCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            MarvRemindersCommand.list.rawValue,
            MarvRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            MarvMotionCommand.activity.rawValue,
            MarvMotionCommand.pedometer.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarvWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = MarvWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarvWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(MarvWatchNotifyParams.self, from: req.paramsJSON)
            let title = params.title.trimmingCharacters(in: .whitespacesAndNewlines)
            let body = params.body.trimmingCharacters(in: .whitespacesAndNewlines)
            if title.isEmpty && body.isEmpty {
                return BridgeInvokeResponse(
            ...
            do {
                ...
                let payload = MarvWatchNotifyPayload(
    ...
    func locationMode() -> MarvLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return MarvLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                        ...
                        await MainActor.run {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). Approve on gateway and return to Marv."
                            } else {
                                self.gatewayStatusText = "Pairing required. Approve on gateway and return to Marv."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "marv-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Marv Share Self-Test",
            url: URL(string: "https://marv.ai/share-self-test"),
