...
enum WatchMessagingError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .watchAppNotInstalled:
            "WATCH_UNAVAILABLE: Marv watch companion app is not installed"
        }
    }
}

final class WatchMessagingService: NSObject, WatchMessagingServicing, @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.marv", category: "watch.messaging")
        ...
        priority: MarvNotificationPriority?) async throws -> WatchNotificationSendResult
    {
        ...
        let payload: [String: Any] = [
            ...
            "title": title,
            "body": body,
            "priority": priority?.rawValue ?? MarvNotificationPriority.active.rawValue,
