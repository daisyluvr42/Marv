...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.marv.gateway"
    private static let nodeService = "ai.marv.node"
    private static let talkService = "ai.marv.talk"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.marv.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.marv.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("marv-gateway.log")
    }
