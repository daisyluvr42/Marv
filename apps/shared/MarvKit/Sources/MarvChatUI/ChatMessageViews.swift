...
struct ChatMessageBubble: View {
    let message: MarvChatMessage
    let style: MarvChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MarvChatTheme.userText : MarvChatTheme.assistantText
    ...
    private var inlineAttachments: [MarvChatMessageContent] {
    ...
    private var toolCalls: [MarvChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MarvChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MarvChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MarvChatTheme.onboardingAssistantBubble
        }
        return MarvChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MarvChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MarvChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MarvChatTheme.userText : MarvChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MarvChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MarvChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MarvChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MarvChatTheme.assistantText)
            }
        }
    }
}
