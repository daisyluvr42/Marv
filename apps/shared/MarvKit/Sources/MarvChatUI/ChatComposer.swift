...
@MainActor
struct MarvChatComposer: View {
    @Bindable var viewModel: MarvChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MarvChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MarvChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MarvPendingAttachment.id)
                { (att: MarvPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MarvPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MarvChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MarvChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MarvChatTheme.subtleCard)
        .clipShape(Capsule())
    }
