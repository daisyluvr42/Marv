...
enum MarvPlatformImageFactory {
    static func image(_ image: MarvPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
