...
private let chatUILogger = Logger(subsystem: "ai.marv", category: "MarvChatUI")

@MainActor
@Observable
public final class MarvChatViewModel {
    public private(set) var messages: [MarvChatMessage] = []
    ...
    public var attachments: [MarvPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MarvChatPendingToolCall] = []
    public private(set) var sessions: [MarvChatSessionEntry] = []
    private let transport: any MarvChatTransport
    ...
    private var pendingToolCallsById: [String: MarvChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MarvChatTransport) {
    ...
    public var sessionChoices: [MarvChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [MarvChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MarvPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MarvChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MarvChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: MarvChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MarvChatMessage],
        incoming: [MarvChatMessage]) -> [MarvChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MarvChatMessage]) -> [MarvChatMessage] {
        var result: [MarvChatMessage] = []
    ...
    private static func dedupeKey(for message: MarvChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MarvChatMessageContent] = [
            MarvChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MarvChatAttachmentPayload in
            MarvChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MarvChatSessionEntry {
        MarvChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MarvChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MarvChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MarvAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MarvChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MarvPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
