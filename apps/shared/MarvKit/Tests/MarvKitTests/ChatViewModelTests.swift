...
private final class TestChatTransport: @unchecked Sendable, MarvChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MarvChatHistoryPayload]
    private let sessionsResponses: [MarvChatSessionsListResponse]

    private let stream: AsyncStream<MarvChatTransportEvent>
    private let continuation: AsyncStream<MarvChatTransportEvent>.Continuation

    init(
        historyResponses: [MarvChatHistoryPayload],
        sessionsResponses: [MarvChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MarvChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MarvChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MarvChatHistoryPayload(
        ...
        attachments _: [MarvChatAttachmentPayload]) async throws -> MarvChatSendResponse
    {
        ...
        return MarvChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MarvChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MarvChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MarvChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MarvChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MarvChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MarvChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MarvChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MarvChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MarvChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MarvChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
