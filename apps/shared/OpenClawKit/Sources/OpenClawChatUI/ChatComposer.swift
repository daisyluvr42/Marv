...
struct OpenClawChatComposer: View {
    ...
    // Shimmer verb animation
    @State private var shimmerVerbs: [String] = ShimmerVerbs.shuffled()
    @State private var shimmerIndex: Int = 0
    ...
    private var currentShimmerVerb: String {
        guard !self.shimmerVerbs.isEmpty else { return "Message Marv…" }
        return self.shimmerVerbs[self.shimmerIndex % self.shimmerVerbs.count] + "…"
    }

    private func startShimmerTimer() {
        self.shimmerTimer?.invalidate()
        self.shimmerTimer = Timer.scheduledTimer(withTimeInterval: 3.5, repeats: true) { _ in
            Task { @MainActor in
                withAnimation(.easeOut(duration: 0.3)) { self.shimmerOpacity = 0 }
                try? await Task.sleep(for: .milliseconds(350))
                self.shimmerIndex += 1
                if self.shimmerIndex >= self.shimmerVerbs.count {
                    self.shimmerVerbs = ShimmerVerbs.shuffled()
                    self.shimmerIndex = 0
                }
                withAnimation(.easeIn(duration: 0.3)) { self.shimmerOpacity = 1 }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 0) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(self.currentShimmerVerb)
                    ...
                    .padding(.vertical, 4)
                    .onAppear { self.startShimmerTimer() }
                    .onDisappear { self.stopShimmerTimer() }
            }
