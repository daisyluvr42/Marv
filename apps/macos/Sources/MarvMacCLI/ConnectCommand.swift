...
struct ConnectOptions {
    ...
    var clientId: String = "marv-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          marv-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: marv-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Marv macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Marv macOS Gateway Connect")
