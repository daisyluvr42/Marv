...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "bot.marv", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Marv run already has an SSH listener on the expected port (common after restarts),
