...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MarvProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.marv", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MarvKit.AnyCodable]()) {
                $0[$1.key] = MarvKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MarvProtocol.AnyCodable?) -> [String: MarvProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MarvProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MarvKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MarvProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("marv.control.heartbeat")
    static let controlAgentEvent = Notification.Name("marv.control.agent")
}
