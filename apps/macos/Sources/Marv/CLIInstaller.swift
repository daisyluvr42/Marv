...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("marv").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing marv CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed marv \($0)." } ?? "Installed marv."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://raw.githubusercontent.com/daisyluvr42/Marv/main/install/install-cli.sh | \
