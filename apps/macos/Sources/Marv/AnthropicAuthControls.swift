...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MarvOAuthStore.AnthropicOAuthStatus = MarvOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MarvOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MarvOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MarvOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Marv can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MarvOAuthStore.AnthropicOAuthStatus,
