...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MARV_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/marv")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("marv-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("marv-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/marv/ (rolling or stdout), or nil if none exist.
