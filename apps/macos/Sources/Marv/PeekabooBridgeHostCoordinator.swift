...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.marv", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MarvPeekabooBridgeServices?
    private static var marvSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Marv", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MarvPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.marvSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.marvSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MarvPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.marv.peekaboo")
