...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.marv", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: MarvNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [MarvCapability.canvas.rawValue, MarvCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(MarvCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if MarvLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(MarvCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            MarvSystemCommand.which.rawValue,
            MarvSystemCommand.run.rawValue,
            MarvSystemCommand.execApprovalsGet.rawValue,
            MarvSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(MarvCapability.camera.rawValue) {
        ...
        if capsSet.contains(MarvCapability.location.rawValue) {
