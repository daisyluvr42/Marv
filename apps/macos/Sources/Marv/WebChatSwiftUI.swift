...
private let webChatSwiftLogger = Logger(subsystem: "ai.marv", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MarvChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MarvChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MarvChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MarvChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MarvGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MarvChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MarvChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MarvChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MarvChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MarvChatView>) -> NSViewController
    {
