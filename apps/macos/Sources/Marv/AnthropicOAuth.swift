...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Marv token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: MarvOAuthStore.AnthropicOAuthStatus = MarvOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "bot.marv", category: "anthropic-oauth")
...
enum MarvOAuthStore {
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Marv OAuth token file not found"
            case .unreadableFile: "Marv OAuth token file not readable"
            case .invalidJSON: "Marv OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Marv OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Marv OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.marvOAuthDirEnv]?
