...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "bot.marv", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["MARV_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = MarvConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        let stored = UserDefaults.standard.integer(forKey: "gatewayPort")
        return stored > 0 ? stored : 4242
    }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.marvExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "marv CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["MARV_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "marv@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "marv@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "marv@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "marv@\(target)"])
            }

        statusHandler("Installing marv@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed marv@\(target)")
        } else {
