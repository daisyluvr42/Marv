...
enum CommandResolver {
    private static let projectRootDefaultsKey = "marv.gatewayProjectRootPath"
    private static let helperName = "marv"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let marvEntry = root.appendingPathComponent("marv.mjs").path
        if FileManager().isReadableFile(atPath: marvEntry) { return marvEntry }
        let binEntry = root.appendingPathComponent("bin/marv.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/marv")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let marvPaths = self.marvManagedPaths(home: home)
        if !marvPaths.isEmpty {
            extras.insert(contentsOf: marvPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + marvPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func marvManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".marv"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func marvExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMarvExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("marv.mjs").path,
            root.appendingPathComponent("bin/marv.js").path,
        ]
    ...
    static func hasAnyMarvInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.marvExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let marvPath = self.projectMarvExecutable(projectRoot: root) {
                return [marvPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "marv", subcommand] + extraArgs
            }
            if let marvPath = self.marvExecutable(searchPaths: searchPaths) {
                return [marvPath, subcommand] + extraArgs
            }

            let missingEntry = """
            marv entrypoint missing (looked for dist/index.js or marv.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real marv CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/marv"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/marv.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/marv.mjs"
            node "$PRJ/marv.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/marv.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/marv.js"
            node "$PRJ/bin/marv.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent marv"
          pnpm --silent marv \(quotedArgs);
        else
          echo "marv CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MarvConfigFile.loadDict()
