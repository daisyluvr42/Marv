...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.marv", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MarvSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MarvSessionPreviewEntry,
    ...
    private static func cache(payload: MarvSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MarvChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MarvKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MarvKit.AnyCodable]) -> [MarvChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MarvChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MarvChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MarvChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MarvChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MarvChatMessage) -> String? {
