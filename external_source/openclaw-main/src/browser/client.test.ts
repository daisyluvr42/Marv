...
describe("browser client", () => {
  ...
  it("wraps connection failures with a sandbox hint", async () => {
    ...
    await expect(browserStatus("http://127.0.0.1:4244")).rejects.toThrow(/sandboxed session/i);
  });

  it("adds useful timeout messaging for abort-like failures", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("aborted")));
    await expect(browserStatus("http://127.0.0.1:4244")).rejects.toThrow(/timed out/i);
  });
  ...
  it("surfaces non-2xx responses with body text", async () => {
    ...
    await expect(
      browserSnapshot("http://127.0.0.1:4244", { format: "aria", limit: 1 }),
  ...
  it("adds labels + efficient mode query params to snapshots", async () => {
    ...
    await expect(
      browserSnapshot("http://127.0.0.1:4244", {
        format: "ai",
        labels: true,
        mode: "efficient",
      }),
  ...
  it("adds refs=aria to snapshots when requested", async () => {
    ...
    await browserSnapshot("http://127.0.0.1:4244", {
      format: "ai",
      refs: "aria",
    });
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    await expect(browserStatus("http://127.0.0.1:4244")).resolves.toMatchObject({
      running: true,
      cdpPort: 18792,
    });

    await expect(browserTabs("http://127.0.0.1:4244")).resolves.toHaveLength(1);
    await expect(
      browserOpenTab("http://127.0.0.1:4244", "https://example.com"),
    ...
    await expect(
      browserNavigate("http://127.0.0.1:4244", { url: "https://example.com" }),
    ).resolves.toMatchObject({ ok: true, targetId: "t1" });
    await expect(
      browserAct("http://127.0.0.1:4244", { kind: "click", ref: "1" }),
    ).resolves.toMatchObject({ ok: true, targetId: "t1" });
    await expect(
      browserArmFileChooser("http://127.0.0.1:4244", {
        paths: ["/tmp/a.txt"],
      }),
    ).resolves.toMatchObject({ ok: true });
    await expect(
      browserArmDialog("http://127.0.0.1:4244", { accept: true }),
    ).resolves.toMatchObject({ ok: true });
    await expect(
      browserConsoleMessages("http://127.0.0.1:4244", { level: "error" }),
    ).resolves.toMatchObject({ ok: true, targetId: "t1" });
    await expect(browserPdfSave("http://127.0.0.1:4244")).resolves.toMatchObject({
      ok: true,
      path: "/tmp/a.pdf",
    });
    await expect(
      browserScreenshotAction("http://127.0.0.1:4244", { fullPage: true }),
