...
最快聊天：打开 Control UI（无需渠道设置）。运行 `openclaw dashboard` 并在浏览器中聊天，或在 Gateway 网关主机上打开 `http://127.0.0.1:4242/`。文档：[Dashboard](/web/dashboard) 和 [Control UI](/web/control-ui)。
...
```bash
node openclaw.mjs gateway --port 4242 --verbose
```
