...
- **Localhost**：打开 `http://127.0.0.1:4242/`。如果你看到"unauthorized"，运行 `openclaw dashboard` 并使用带 token 的链接（`?token=...`）。
...
- 确保 Gateway 网关可达（本地：`openclaw status`；远程：SSH 隧道 `ssh -N -L 4242:127.0.0.1:4242 user@host` 然后打开 `http://127.0.0.1:4242/?token=...`）。
