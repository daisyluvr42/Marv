...
- 默认在 `canvasHost.port`（默认 `4246`）上启动 Canvas 文件服务器，从 `~/.openclaw/workspace/canvas` 提供 `http://<gateway-host>:4246/__openclaw__/canvas/`。使用 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- 端口优先级：`--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > 默认 `4242`。
...
[Service]
ExecStart=/usr/local/bin/openclaw gateway --port 4242
Restart=always
RestartSec=5
