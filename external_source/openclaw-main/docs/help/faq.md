...
- **Tailnet bind**: run `openclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:4242/`, paste token in dashboard settings.
...
```
--port > OPENCLAW_GATEWAY_PORT > gateway.port > default 4242
```
...
OpenClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:4242`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.
...
```
openclaw tui --url ws://<host>:4242 --token <token>
```
