...
const URL_PARAM_PATTERN =
  /([?&](?:access_token|accesstoken|appsecret|app_secret|clientsecret|client_secret|token)=)([^&#\s]+)/gi;
const KV_PATTERN =
  /((?:access[_-]?token|app[_-]?secret|app[_-]?key|client[_-]?secret|client[_-]?id|bot[_-]?token|token)\s*[:=]\s*['"]?)([^'",\s}]+)/gi;
...
function isSensitiveKey(key: string): boolean {
  ...
  return (
    SENSITIVE_KEYS.has(normalized) || normalized.endsWith("secret") || normalized.endsWith("token")
  );
}
...
export function sanitizeSensitiveString(value: string): string {
  return value
    .replace(
      URL_PARAM_PATTERN,
      (_match, prefix: string, secret: string) => `${prefix}${maskSecretMiddle(secret)}`,
    )
    .replace(
      KV_PATTERN,
      (_match, prefix: string, secret: string) => `${prefix}${maskSecretMiddle(secret)}`,
