...
export function collectSkillFilesForSandbox(skillsRoot: string): { path: string; data: Buffer }[] {
  ...
  function scan(dir: string, base: string): void {
    ...
    for (const entry of entries) {
      if (SKILL_SYNC_IGNORE.has(entry.name)) {
        continue;
      }
        ...
        try {
          ...
          if (stat.size <= SKILL_SYNC_MAX_FILE_SIZE) {
            ...
            coworkLog(
              "WARN",
              "collectSkillFiles",
              `Skipping oversized file: ${relPath} (${stat.size} bytes)`,
            );
          }
        } catch {
...
function buildQemuArgs(options: {
  ...
  const args: string[] = ["-m", "4096", "-smp", "2", "-nographic", "-snapshot"];
  ...
  if (options.runtime.arch === "arm64") {
    ...
    const hasKernel = Boolean(
      kernelPath && initrdPath && fs.existsSync(kernelPath) && fs.existsSync(initrdPath),
    );
    ...
    if (hasKernel) {
      ...
      if (firmware) {
        args.push(
          "-drive",
          `if=pflash,format=raw,readonly=on,file=${firmware.codePath}`,
          "-drive",
          `if=pflash,format=raw,file=${firmware.varsPath}`,
        );
      }
    }
  }
  ...
  args.push(
    "-drive",
    `file=${options.runtime.imagePath},if=virtio,format=qcow2`,
    "-netdev",
    "user,id=net0",
    "-device",
  ...
  if (options.runtime.platform === "win32") {
    ...
    args.push("-virtfs", `local,path=${options.ipcDir},mount_tag=ipc,security_model=none`);
    args.push(
      "-virtfs",
    ...
    if (!hasExplicitExtraMounts && options.skillsDir && fs.existsSync(options.skillsDir)) {
      args.push("-virtfs", `local,path=${options.skillsDir},mount_tag=skills,security_model=none`);
    }
  }

  args.push("-serial", `file:${path.join(options.ipcDir, "serial.log")}`);
...
export function spawnCoworkSandboxVm(options: {
  ...
  if (options.launcher === "launchctl" && process.platform === "darwin") {
    ...
    if (uid !== null) {
      return spawn(
        "/bin/launchctl",
        ["asuser", String(uid), options.runtime.runtimeBinary, ...args],
        {
...
export class VirtioSerialBridge {
  ...
  private handleFileSyncChunk(msg: Record<string, unknown>): void {
    ...
    if (!transferId || !relativePath || !data) {
      return;
    }

    // Validate path early
    if (!this.resolveHostPath(relativePath)) {
      return;
    }
  ...
  pushFile(basePath: string, relativePath: string, data: Buffer): void {
    coworkLog(
      "INFO",
      "VirtioSerialBridge",
      `pushFile: ${relativePath} (${data.length} bytes) -> ${basePath}/${relativePath}`,
    );
  ...
  private sendLine(data: Record<string, unknown>): void {
    if (this.socket && this.connected) {
      ...
      coworkLog(
        "WARN",
        "VirtioSerialBridge",
        `sendLine dropped (not connected): type=${String(data.type ?? "unknown")}`,
      );
    }
  }
