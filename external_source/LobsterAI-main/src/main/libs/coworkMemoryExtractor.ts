...
const NON_DURABLE_TOPIC_RE =
  /(我有\s*(?:一个|个)?问题|有个问题|报错|出现异常|exception|stack\s*trace)/i;
...
export function isQuestionLikeMemoryText(text: string): boolean {
  ...
  if (CHINESE_QUESTION_PREFIX_RE.test(normalized)) {
    return true;
  }
  if (ENGLISH_QUESTION_PREFIX_RE.test(normalized)) {
    return true;
  }
  if (QUESTION_INLINE_RE.test(normalized)) {
    return true;
  }
...
function shouldKeepCandidate(text: string): boolean {
  ...
  if (trimmed.length < 6 && !SHORT_FACT_SIGNAL_RE.test(trimmed)) {
    return false;
  }
  if (SMALL_TALK_RE.test(trimmed)) {
    return false;
  }
  if (isQuestionLikeMemoryText(trimmed)) {
    return false;
  }
  if (ASSISTANT_STYLE_CANDIDATE_RE.test(trimmed)) {
    return false;
  }
  if (PROCEDURAL_CANDIDATE_RE.test(trimmed)) {
    return false;
  }
  return true;
}
...
function extractImplicit(options: ExtractTurnMemoryOptions): ExtractedMemoryChange[] {
  ...
  if (!strippedUser || !strippedAssistant) {
    return [];
  }
  ...
  for (const rawCandidate of candidates) {
    ...
    seen.add(key);
    if (NON_DURABLE_TOPIC_RE.test(candidate)) {
      continue;
    }
    ...
    if (result.length >= maxImplicitAdds) {
      break;
    }
  }
...
export function extractTurnMemoryChanges(
  options: ExtractTurnMemoryOptions,
): ExtractedMemoryChange[] {
  ...
  const explicitDeletes = extractExplicit(
    userText,
    "delete",
    EXPLICIT_DELETE_RE,
