...
function buildCharacterBigramMap(value: string): Map<string, number> {
  ...
  if (compact.length <= 1) {
    return new Map<string, number>([[compact, 1]]);
  }
...
export class CoworkStore {
  ...
  getConfig(): CoworkConfig {
    ...
    const workingDirRow = this.getOne<ConfigRow>("SELECT value FROM cowork_config WHERE key = ?", [
      "workingDirectory",
    ]);
    const executionModeRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["executionMode"],
    );
    const memoryEnabledRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["memoryEnabled"],
    );
    const memoryImplicitUpdateEnabledRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["memoryImplicitUpdateEnabled"],
    );
    const memoryLlmJudgeEnabledRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["memoryLlmJudgeEnabled"],
    );
    const memoryGuardLevelRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["memoryGuardLevel"],
    );
    const memoryUserMemoriesMaxItemsRow = this.getOne<ConfigRow>(
      "SELECT value FROM cowork_config WHERE key = ?",
      ["memoryUserMemoriesMaxItems"],
    );

    const normalizedExecutionMode =
      executionModeRow?.value === "container"
        ? "sandbox"
    ...
    return {
      ...
      memoryUserMemoriesMaxItems: clampMemoryUserMemoriesMaxItems(
        Number(memoryUserMemoriesMaxItemsRow?.value),
      ),
    };
  }
