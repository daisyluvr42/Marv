...
const TaskDetail: React.FC<TaskDetailProps> = ({ task, onRequestDelete }) => {
  ...
  const statusLabel = task.state.lastStatus
    ? i18nService.t(
        `scheduledTasksStatus${task.state.lastStatus.charAt(0).toUpperCase() + task.state.lastStatus.slice(1)}`,
      )
    : "-";
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className={valueClass}>
              <span
                className={`inline-flex items-center gap-1 ${task.enabled ? "text-green-500" : "dark:text-claude-darkTextSecondary text-claude-textSecondary"}`}
              >
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className={valueClass}>
              {task.state.lastStatus && (
                <span className={statusColor[task.state.lastStatus] || ""}>{statusLabel}</span>
              )}
              {!task.state.lastStatus && "-"}
          ...
          <div>
            ...
            <div className={valueClass}>
              {task.state.nextRunAtMs ? new Date(task.state.nextRunAtMs).toLocaleString() : "-"}
            </div>
          </div>
