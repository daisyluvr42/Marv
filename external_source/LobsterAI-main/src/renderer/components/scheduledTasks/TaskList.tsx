...
const TaskListItem: React.FC<TaskListItemProps> = ({ task, onRequestDelete }) => {
  ...
  return (
    ...
    >
      {/* Title */}
      <div
        className={`text-sm truncate ${task.enabled ? "dark:text-claude-darkText text-claude-text" : "dark:text-claude-darkTextSecondary text-claude-textSecondary"}`}
      >
        {task.name}
      </div>
