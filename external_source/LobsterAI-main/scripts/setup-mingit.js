...
async function resolveArchive(required) {
  ...
  if (envArchive) {
    if (!isNonEmptyFile(envArchive)) {
      throw new Error(`LOBSTERAI_PORTABLE_GIT_ARCHIVE points to an invalid file: ${envArchive}`);
    }
    console.log(
      `[setup-mingit] Using local archive from LOBSTERAI_PORTABLE_GIT_ARCHIVE: ${envArchive}`,
    );
  ...
  const urlFromEnv =
    typeof process.env.LOBSTERAI_PORTABLE_GIT_URL === "string"
...
async function ensurePortableGit(options = {}) {
  ...
  const shouldRun =
    process.platform === "win32" || required || process.env.LOBSTERAI_SETUP_MINGIT_FORCE === "1";
