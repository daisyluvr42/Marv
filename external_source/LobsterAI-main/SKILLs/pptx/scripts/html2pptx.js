...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const SINGLE_WEIGHT_FONTS = new Set(["impact"]);

    // Helper: Check if a font should skip bold formatting
    const shouldSkipBold = (fontFamily) => {
      if (!fontFamily) {return false;}
      const normalizedFont = fontFamily.toLowerCase().replace(/['"]/g, "").split(",")[0].trim();
      return SINGLE_WEIGHT_FONTS.has(normalizedFont);
    };
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      const isContainer = el.tagName === "DIV" && !textTags.has(el.tagName);
      if (isContainer) {
        const computed = window.getComputedStyle(el);
        const hasBg = computed.backgroundColor && computed.backgroundColor !== "rgba(0, 0, 0, 0)";
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    if (!error.message.startsWith(htmlFile)) {
      throw new Error(`${htmlFile}: ${error.message}`, { cause: error });
    }
    throw error;
  }
}
