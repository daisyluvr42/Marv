  ...
  async search(
    ...
    try {
      ...
      try {
        ...
        if (isBlocked) {
          throw new Error('Google appears blocked or unavailable from this network', { cause: error });
        }
        throw new Error('Google search results did not load in time', { cause: error });
      }
      ...
      throw new Error(`Google search failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
    }
  }
