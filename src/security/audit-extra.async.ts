...
export async function collectPluginsTrustFindings(params: {
  ...
  if (enabledExtensionPluginIds.length > 0) {
    ...
    for (const context of contexts) {
      ...
      const broadPolicy = isToolAllowedByPolicies("__marv_plugin_probe__", policies);
...
export async function collectStateDeepFilesystemFindings(params: {
  ...
  const ids = [normalizeAgentId(defaultAgentId)];
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `marv security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all marv.extensions entries stay inside the plugin directory (legacy marv.extensions also supported).",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Marv extensions state directory.",
      });
    ...
    for (const entry of entries) {
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `marv security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
