...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep MARV_STATE_DIR on a local-only volume (legacy OPENCLAW_STATE_DIR also supported) and re-run "${formatCliCommand("marv security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer MARV_GATEWAY_PASSWORD (env; legacy OPENCLAW_GATEWAY_PASSWORD also supported) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-marv-session-key (legacy x-openclaw-session-key also supported) for per-request session routing. ` +
