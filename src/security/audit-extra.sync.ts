...
function summarizeGroupPolicy(cfg: MarvConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: MarvConfig): boolean {
...
function collectModels(cfg: MarvConfig): ModelRef[] {
  ...
  for (const fallback of cfg.agents?.defaults?.model?.fallbacks ?? []) {
...
function listKnownNodeCommands(cfg: MarvConfig): Set<string> {
  const baseCfg: MarvConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: MarvConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: MarvConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: MarvConfig): boolean {
...
function isBrowserEnabled(cfg: MarvConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: MarvConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: MarvConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep MARV_STATE_DIR on a local-only volume (legacy MARV_STATE_DIR also supported) and re-run "${formatCliCommand("marv security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: MarvConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer MARV_GATEWAY_PASSWORD (env; legacy MARV_GATEWAY_PASSWORD also supported) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const marvGatewayToken =
    typeof env.MARV_GATEWAY_TOKEN === "string" && env.MARV_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-marv-session-key (legacy x-marv-session-key also supported) for per-request session routing. ` +
...
export function collectNodeDenyCommandPatternFindings(cfg: MarvConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: MarvConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: MarvConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: MarvConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
