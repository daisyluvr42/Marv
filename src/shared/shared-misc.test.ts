...
describe("shared/frontmatter", () => {
  ...
  test("resolveMarvManifestBlock parses JSON5 metadata and picks marv block", () => {
    const frontmatter = {
      metadata: "{ marv: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveMarvManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
