    ...
    if (addressInUse) {
      log.warn(
        "gog serve failed to bind (address already in use); stopping restarts. " +
          "Another watcher is likely running. Set MARV_SKIP_GMAIL_WATCHER=1 (legacy MARV_SKIP_GMAIL_WATCHER=1 also works) or stop the other process.",
      );
      watcherProcess = null;
      return;
    }
 ...
 */
export async function startGmailWatcher(cfg: MarvConfig): Promise<GmailWatcherStartResult> {
