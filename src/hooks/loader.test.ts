...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marv-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.MARV_BUNDLED_HOOKS_DIR;
    process.env.MARV_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.MARV_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MARV_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
