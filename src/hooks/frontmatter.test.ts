...
`;
    ...
    expect(parsed.marv.emoji).toBe("💾");
    expect(parsed.marv.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.marv.emoji).toBe("📝");
    expect(parsed.marv.events).toEqual(["command"]);
    expect(parsed.marv.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.marv.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"marv": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"marv": {"events": ["test"]}}');
  });
...
describe("resolveMarvMetadata", () => {
  it("extracts marv metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMarvMetadata(frontmatter);
  ...
  it("returns undefined when marv key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        marv: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Marv" },
            { id: "npm", kind: "npm", package: "@marv/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@marv/hook");
  });
...
`;
    ...
    const marv = resolveMarvMetadata(frontmatter);
    expect(marv).toBeDefined();
    expect(marv?.emoji).toBe("💾");
    expect(marv?.events).toEqual(["command:new"]);
    expect(marv?.requires?.config).toEqual(["workspace.dir"]);
    expect(marv?.install?.[0].kind).toBe("bundled");
  });
