...
2. **Appends to log file** - Writes a JSON line to `~/.marv/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.marv/logs/commands.log ~/.marv/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/marv`:
   ```
   /home/username/.marv/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
