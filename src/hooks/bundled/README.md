...
This directory contains hooks that ship with Marv. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Events**: `command:new`
**What it does**: Captures recent conversation context and writes a structured `session_summary` entry to soul memory.
**Output**: `soul-memory/<id>` reference in logs.
...
Hooks can be configured in `~/.marv/marv.json`:
...
2. Restart gateway: `pkill -9 -f 'marv.*gateway' && pnpm marv gateway`
3. Enable the hook: `marv hooks enable my-hook`
