 ...
 *
 * Saves session context to structured soul memory when /new is triggered.
 */
...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    // Build structured summary entry
    const entryParts = [
      `session_date=${dateStr}`,
      `session_time_utc=${timeStr}`,
      `session_topic_slug=${slug}`,
    ...
    if (sessionContent) {
      entryParts.push("", "conversation_summary:", sessionContent);
    }
