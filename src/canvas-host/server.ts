...
(() => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Marv node canvas.");
      return;
    }
  ...
  const handleHttpRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      const effectiveBasePath =
        ...
        (urlPath === LEGACY_CANVAS_HOST_PATH || urlPath.startsWith(`${LEGACY_CANVAS_HOST_PATH}/`))
          ? LEGACY_CANVAS_HOST_PATH
          : basePath;
      if (effectiveBasePath !== "/") {
        if (urlPath !== effectiveBasePath && !urlPath.startsWith(`${effectiveBasePath}/`)) {
          return false;
        }
        urlPath =
          urlPath === effectiveBasePath ? "/" : urlPath.slice(effectiveBasePath.length) || "/";
      }
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Marv Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
