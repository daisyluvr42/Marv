...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "marv-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateMarvProfile(userDataDir, { color: DEFAULT_MARV_BROWSER_COLOR });
    ...
    expect(def.name).toBe(DEFAULT_MARV_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_MARV_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(path.join(userDataDir, ".marv-profile-decorated"), "utf-8");
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_MARV_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopMarvChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopMarvChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopMarvChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopMarvChrome sends SIGTERM and returns once CDP is down", async () => {
