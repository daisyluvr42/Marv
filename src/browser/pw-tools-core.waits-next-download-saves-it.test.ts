...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMarvTmpDir: vi.fn(() => "/tmp/marv"),
}));
vi.mock("../infra/tmp-marv-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMarvTmpDir.mockReturnValue("/tmp/marv");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMarvTmpDir.mockReturnValue("/tmp/marv-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "marv-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "marv-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMarvTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "marv-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "marv-preferred", "downloads")}${path.sep}`),
    );
  });
