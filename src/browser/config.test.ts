...
describe("browser config", () => {
  ...
  it("derives default ports from MARV_GATEWAY_PORT when unset", () => {
    const prev = process.env.MARV_GATEWAY_PORT;
    process.env.MARV_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const marv = resolveProfile(resolved, "marv");
      expect(marv?.cdpPort).toBe(19012);
      expect(marv?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.MARV_GATEWAY_PORT;
      } else {
        process.env.MARV_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const marv = resolveProfile(resolved, "marv");
      expect(marv?.cdpPort).toBe(19022);
      expect(marv?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.MARV_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "marv");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    ...
    expect(resolved.defaultProfile).toBe("marv");
  });
