...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/marv" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "marv",
        profiles: {
          marv: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMarvChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveMarvUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopMarvChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.MARV_GATEWAY_PORT;
    process.env.MARV_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.MARV_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.MARV_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.MARV_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.MARV_GATEWAY_TOKEN;
    } else {
      process.env.MARV_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.MARV_GATEWAY_PASSWORD;
    } else {
      process.env.MARV_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
