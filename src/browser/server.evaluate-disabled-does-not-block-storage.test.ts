  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "marv",
        profiles: {
          marv: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MARV_GATEWAY_PORT;
    process.env.MARV_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MARV_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MARV_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.MARV_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.MARV_GATEWAY_TOKEN;
    } else {
      process.env.MARV_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.MARV_GATEWAY_PASSWORD;
    } else {
      process.env.MARV_GATEWAY_PASSWORD = prevGatewayPassword;
    }
