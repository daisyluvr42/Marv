...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the Marv gateway (Marv.app menubar, or \`${formatCliCommand("marv gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the Marv browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the Marv browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
