...
import {
  ...
  LEGACY_MARV_BROWSER_PROFILE_NAME,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_MARV_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "marv" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MARV_BROWSER_PROFILE_NAME] && !result[LEGACY_MARV_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MARV_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Marv browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for marv/marv CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MarvConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MARV_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_MARV_BROWSER_PROFILE_NAME]
        ? DEFAULT_MARV_BROWSER_PROFILE_NAME
        : profiles[LEGACY_MARV_BROWSER_PROFILE_NAME]
          ? LEGACY_MARV_BROWSER_PROFILE_NAME
          : DEFAULT_MARV_BROWSER_PROFILE_NAME);
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "marv";
