...
function getRelayHeader(req: IncomingMessage): string | undefined {
  return getHeader(req, RELAY_AUTH_HEADER) ?? getHeader(req, LEGACY_RELAY_AUTH_HEADER);
}
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Marv-Extension-Relay",
          revision: "0",
          userAgent: "Marv-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (path.startsWith("/json")) {
      const token = getRelayHeader(req);
