...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "marv-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "marv-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "marv-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.marv when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "marv-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers MARV_HOME over HOME", () => {
    vi.stubEnv("MARV_HOME", "/srv/marv-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/marv-home"));
...
describe("shortenHomePath", () => {
  it("uses $MARV_HOME prefix when MARV_HOME is set", () => {
    vi.stubEnv("MARV_HOME", "/srv/marv-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/marv-home")}/.marv/marv.json`)).toBe(
      "$MARV_HOME/.marv/marv.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MARV_HOME replacement when MARV_HOME is set", () => {
    vi.stubEnv("MARV_HOME", "/srv/marv-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/marv")).toBe(path.resolve(os.homedir(), "marv"));
  });
  ...
  it("prefers MARV_HOME for tilde expansion", () => {
    vi.stubEnv("MARV_HOME", "/srv/marv-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/marv")).toBe(path.resolve("/srv/marv-home", "marv"));
