...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const envSnapshot = captureEnv(["MARV_STATE_DIR", "MARV_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-oauth-"));

    try {
      process.env.MARV_STATE_DIR = tempDir;
      process.env.MARV_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.MARV_AGENT_DIR, {
