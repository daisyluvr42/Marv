...
async function runEnvProviderCase(params: {
  ...
  try {
    await ensureMarvModelsJson({});

    const modelPath = path.join(resolveMarvAgentDir(), "models.json");
...
describe("models-config", () => {
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      await withTempEnv([...MODELS_CONFIG_IMPLICIT_ENV_VARS, "KIMI_API_KEY"], async () => {
        ...
        const result = await ensureMarvModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      await ensureMarvModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
