...
export function persistGoalStrategyMemory(params: {
  ...
  if (!params.state.convergeReason || params.state.convergeReason !== "sufficient_completion") {
    return;
  }
  ...
  writeSoulMemory({
    ...
    content,
    summary: `${params.state.strategyFamily} worked for ${params.state.goalFrame.objective}`,
