...
function restoreMarvStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.MARV_STATE_DIR;
    return;
  }
  process.env.MARV_STATE_DIR = originalValue;
}

const originalStateDir = process.env.MARV_STATE_DIR;

afterEach(() => {
  restoreMarvStateDir(originalStateDir);
});
