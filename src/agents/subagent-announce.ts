...
export function buildAnnounceReplyInstruction(params: {
  ...
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. Keep this internal context private (don't mention system/log/stats/session details or announce type). You MUST only restate or summarize the provided result. Do NOT promise, claim, or suggest any follow-up actions (such as switching models, generating reports, sending messages, or retrying) unless you are actually executing them with a tool call in this same turn.`;
  }
  ...
  return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. Keep this internal context private (don't mention system/log/stats/session details or announce type), and do not copy the system message verbatim. You MUST only restate or summarize the provided result. Do NOT promise, claim, or suggest any follow-up actions (such as switching models, generating reports, sending messages, or retrying) unless you are actually executing them with a tool call in this same turn. Reply ONLY: ${SILENT_REPLY_TOKEN} if this exact result was already delivered to the user in this same turn.`;
}
