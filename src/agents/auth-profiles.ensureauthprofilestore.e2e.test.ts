...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marv-auth-merge-"));
    const previousAgentDir = process.env.MARV_AGENT_DIR;
    ...
    try {
      ...
      process.env.MARV_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.MARV_AGENT_DIR = previousAgentDir;
      }
