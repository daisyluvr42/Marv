...
import {
  ...
  LEGACY_DEFAULT_SANDBOX_BROWSER_IMAGE,
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_MARV_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_MARV_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_MARV_BROWSER_PROFILE_NAME]: {
...
async function ensureSandboxBrowserImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_BROWSER_IMAGE) {
    const legacy = await execDocker(["image", "inspect", LEGACY_DEFAULT_SANDBOX_BROWSER_IMAGE], {
      allowFailure: true,
    });
    if (legacy.code === 0) {
      await execDocker([
...
export async function ensureSandboxBrowser(params: {
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `marv sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `marv sandbox recreate --browser --agent ${agentId}`;
          }
          return "marv sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    if (params.cfg.workspaceAccess !== "none" && params.workspaceDir !== params.agentWorkspaceDir) {
      const agentMountSuffix =
        params.cfg.workspaceAccess === "ro" || params.cfg.workspaceAccess === "isolated-rw"
          ? ":ro"
          : "";
      args.push(
        "-v",
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_MARV_BROWSER_PROFILE_NAME)
    : null;
