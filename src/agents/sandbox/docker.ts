...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    const legacyExists = await dockerImageExists(LEGACY_DEFAULT_SANDBOX_IMAGE).catch(() => false);
    if (legacyExists) {
      await execDocker(["tag", LEGACY_DEFAULT_SANDBOX_IMAGE, DEFAULT_SANDBOX_IMAGE]);
      return;
    }
...
async function createSandboxContainer(params: {
  ...
  if (params.workspaceAccess !== "none" && workspaceDir !== params.agentWorkspaceDir) {
    const agentMountSuffix =
      params.workspaceAccess === "ro" || params.workspaceAccess === "isolated-rw" ? ":ro" : "";
    args.push(
      "-v",
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`marv sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`marv sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("marv sandbox recreate --all");
}
