...
describe("buildSandboxFsMounts", () => {
  it("keeps sandbox workspace writable but mounts /agent read-only for isolated-rw", () => {
    const sandbox = createSandbox({
    ...
    const mounts = buildSandboxFsMounts(sandbox);
    const workspaceMount = mounts.find((mount) => mount.source === "workspace");
    const agentMount = mounts.find((mount) => mount.source === "agent");

    expect(workspaceMount?.hostRoot).toBe(path.resolve("/tmp/sandbox-workspace"));
    expect(workspaceMount?.writable).toBe(true);
    expect(agentMount?.hostRoot).toBe(path.resolve("/tmp/agent-workspace"));
    expect(agentMount?.writable).toBe(false);
  });
});
