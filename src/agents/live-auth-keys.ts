...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MARV_LIVE_ANTHROPIC_KEY",
    listVar: "MARV_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MARV_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MARV_LIVE_${base}_KEY`;
