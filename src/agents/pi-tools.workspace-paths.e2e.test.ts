...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("marv-ws-", async (workspaceDir) => {
      await withTempDir("marv-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMarvCodingTools({ workspaceDir });
    ...
    await withTempDir("marv-ws-", async (workspaceDir) => {
      await withTempDir("marv-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello marv");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("marv-ws-", async (workspaceDir) => {
      await withTempDir("marv-override-", async (overrideDir) => {
        const tools = createMarvCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("marv-sandbox-", async (sandboxDir) => {
      await withTempDir("marv-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createMarvCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
