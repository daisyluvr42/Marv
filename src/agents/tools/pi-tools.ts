...
import type { ModelAuthMode } from "../model/model-auth.js";
...
function resolveExecConfig(params: { cfg?: MarvConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: MarvConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createMarvCodingTools(options?: {
  ...
  const allowWorkspaceWrites = sandbox
    ? sandbox.workspaceAccess === "rw" || sandbox.workspaceAccess === "isolated-rw"
    : true;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createMarvReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...createMarvTools({
      ...
      enableProactiveTasks: options?.enableProactiveTasks,
