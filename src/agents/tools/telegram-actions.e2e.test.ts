...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): MarvConfig {
    return {
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as MarvConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: MarvConfig, accountId = "media") {
    await handleTelegramAction(
