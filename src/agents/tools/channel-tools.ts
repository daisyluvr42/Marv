...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as MarvConfig);
...
export function listChannelAgentTools(params: { cfg?: MarvConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: MarvConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
