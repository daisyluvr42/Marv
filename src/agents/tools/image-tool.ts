...
import { getDefaultLocalRoots, loadWebMedia } from "../../channels/web/media.js";
...
function resolveDefaultModelRef(cfg?: MarvConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: MarvConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: MarvConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureMarvModelsJson(effectiveCfg, params.agentDir);
