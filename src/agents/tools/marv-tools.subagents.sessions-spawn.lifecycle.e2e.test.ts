...
type CreateMarvToolsOpts = Parameters<CreateMarvTools>[0];

async function getSessionsSpawnTool(opts: CreateMarvToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createMarvTools } = await import("./marv-tools.js");
  const tool = createMarvTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("marv-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
