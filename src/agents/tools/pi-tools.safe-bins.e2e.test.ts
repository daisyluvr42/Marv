...
const bundledPluginsDirSnapshot = captureEnv(["MARV_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.MARV_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "marv-test-no-bundled-extensions");
});
...
describe("createMarvCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    if (process.platform === "win32") {
      return;
    }

    const { createMarvCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-safe-bins-"));
    ...
    const envSnapshot = captureEnv(["MARV_SHELL_ENV_TIMEOUT_MS"]);
    const result = await (async () => {
      try {
        process.env.MARV_SHELL_ENV_TIMEOUT_MS = "1000";
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    ...
    const { createMarvCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-safe-bins-expand-"));
