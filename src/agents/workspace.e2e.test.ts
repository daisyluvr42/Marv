...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MARV_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MARV_HOME: "/srv/marv-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/marv-home"), ".marv", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".marv", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("marv-workspace-");
