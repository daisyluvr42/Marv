...
export function resolveDefaultAgentWorkspaceDir(
  ...
  return path.join(home, ".marv", "workspace");
}
...
function resolveWorkspaceStatePath(dir: string, dirname = WORKSPACE_STATE_DIRNAME): string {
  return path.join(dir, dirname, WORKSPACE_STATE_FILENAME);
}
...
async function readWorkspaceOnboardingStateForDir(dir: string): Promise<WorkspaceOnboardingState> {
  ...
  const primaryStatePath = resolveWorkspaceStatePath(resolvedDir, WORKSPACE_STATE_DIRNAME);
  const primaryState = await readWorkspaceOnboardingState(primaryStatePath);
  if (primaryState.bootstrapSeededAt || primaryState.onboardingCompletedAt) {
    return primaryState;
  }

  const legacyStatePath = resolveWorkspaceStatePath(resolvedDir, LEGACY_WORKSPACE_STATE_DIRNAME);
  const legacyState = await readWorkspaceOnboardingState(legacyStatePath);
  if (legacyState.bootstrapSeededAt || legacyState.onboardingCompletedAt) {
    return legacyState;
  }
