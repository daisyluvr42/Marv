...
function listAgents(cfg: MarvConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MarvConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MarvConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MarvConfig, EvaluatedBindingsCache>();
