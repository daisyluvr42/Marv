...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "marv-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MARV_GATEWAY_TOKEN: "test-token",
    MARV_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MARV_WORKSPACE_DIR: join(sandbox.rootDir, "marv"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MARV_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MARV_EXTRA_MOUNTS: undefined,
        MARV_HOME_VOLUME: "marv-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MARV_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MARV_EXTRA_MOUNTS=");
    expect(envFile).toContain("MARV_HOME_VOLUME=marv-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("marv-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("marv-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MARV_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
