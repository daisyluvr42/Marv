...
const SLACK_COMMAND_ARG_ACTION_ID = "marv_cmdarg";
const LEGACY_SLACK_COMMAND_ARG_ACTION_ID = "openclaw_cmdarg";
...
const SLACK_COMMAND_ARG_EXTERNAL_PREFIX = "marv_cmdarg_ext:";
const LEGACY_SLACK_COMMAND_ARG_EXTERNAL_PREFIX = "openclaw_cmdarg_ext:";
...
function readSlackExternalArgMenuToken(raw: unknown): string | undefined {
  ...
  const token = raw.startsWith(SLACK_COMMAND_ARG_EXTERNAL_PREFIX)
    ? raw.slice(SLACK_COMMAND_ARG_EXTERNAL_PREFIX.length).trim()
    : raw.startsWith(LEGACY_SLACK_COMMAND_ARG_EXTERNAL_PREFIX)
      ? raw.slice(LEGACY_SLACK_COMMAND_ARG_EXTERNAL_PREFIX.length).trim()
      : "";
...
export async function registerSlackMonitorSlashCommands(params: {
  ...
  const registerArgOptions = () => {
    ...
    for (const actionId of [SLACK_COMMAND_ARG_ACTION_ID, LEGACY_SLACK_COMMAND_ARG_ACTION_ID]) {
      optionsHandler(actionId, async ({ ack, body }) => {
  ...
  registerArgAction(LEGACY_SLACK_COMMAND_ARG_ACTION_ID);
}
