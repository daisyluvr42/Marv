...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "marv-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.MARV_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "marv-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as MarvConfig;
    const res = resolveTelegramToken(cfg);
