...
function createDefaultThreadConfig(): LoadedConfig {
  return {
    ...
    session: { store: "/tmp/marv-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bmarv\\b"] },
        },
      } as ReturnType<typeof import("../../core/config/config.js").loadConfig>;
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Marv" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/marv" },
      session: { store: "/tmp/marv-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
