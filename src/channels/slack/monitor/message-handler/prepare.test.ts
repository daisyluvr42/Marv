...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marv-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: MarvConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/marv" } },
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/marv", enabled: true },
    });
