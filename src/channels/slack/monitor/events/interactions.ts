...
const ACTION_PREFIX_MATCHER = new RegExp(`^(?:${MARV_ACTION_PREFIX}|${LEGACY_MARV_ACTION_PREFIX})`);
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  // Handle Block Kit button clicks from Marv-generated messages
  // Only matches action_ids that start with our prefix to avoid interfering
  // with other Slack integrations or future features
  ctx.app.action(ACTION_PREFIX_MATCHER, async (args: SlackActionMiddlewareArgs) => {
  ...
  // Handle Marv modal submissions with callback_ids scoped by our prefix.
  ctx.app.view(
    ACTION_PREFIX_MATCHER,
