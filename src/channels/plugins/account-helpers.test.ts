...
function cfg(accounts?: Record<string, unknown> | null): MarvConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MarvConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MarvConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MarvConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MarvConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MarvConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MarvConfig)).toBe("default");
    });
  });
});
