...
function telegramCfg(): MarvConfig {
  return { channels: { telegram: { botToken: "tok" } } } as MarvConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as MarvConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as MarvConfig;
