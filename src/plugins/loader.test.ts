...
const fixtureRoot = path.join(os.tmpdir(), `marv-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MARV_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "marv.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MARV_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMarvPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.MARV_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMarvPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadMarvPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
