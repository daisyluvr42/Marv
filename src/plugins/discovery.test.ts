...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `marv-plugins-${randomUUID()}`);
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  const prev = process.env.MARV_STATE_DIR;
  const prevBundled = process.env.MARV_BUNDLED_PLUGINS_DIR;
  ...
  try {
    ...
    if (prev === undefined) {
      ...
      process.env.MARV_STATE_DIR = prev;
    }
    if (prevBundled === undefined) {
      delete process.env.MARV_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.MARV_BUNDLED_PLUGINS_DIR = prevBundled;
    }
  }
}
...
describe("discoverMarvPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".marv", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverMarvPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        marv: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@marv/voice-call",
        marv: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverMarvPlugins({ extraPaths: [packDir] });
    });
