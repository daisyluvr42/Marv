...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `marv-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        marv: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.marv/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without marv.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@marv/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("marv.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@marv/voice-call@0.0.1",
    });
