...
async function ensureManifestExtensions(manifest: PackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json marv.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureManifestExtensions(manifest);
  } catch (err) {
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "marv security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "marv security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(params: {
  ...
  return await withTempDir("marv-plugin-", async (tmpDir) => {
...
export async function installPluginFromNpmSpec(params: {
  ...
  return await withTempDir("marv-npm-pack-", async (tmpDir) => {
