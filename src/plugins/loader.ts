...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MarvPluginDefinition;
  register?: MarvPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MarvPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MarvPluginDefinition;
...
export function loadMarvPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverMarvPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "agentmarv/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? {
                    "agentmarv/plugin-sdk/account-id": pluginSdkAccountIdAlias,
                  }
                : {}),
            },
          }
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MarvPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as MarvPluginModule;
    } catch (err) {
