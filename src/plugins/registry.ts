...
export type PluginHttpRouteRegistration = {
  pluginId?: string;
  path: string;
  handler: MarvPluginHttpRouteHandler;
  source?: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: MarvPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: MarvPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: MarvPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: MarvPluginToolContext) => tool;
  ...
  const registerHttpRoute = (
    record: PluginRecord,
    params: { path: string; handler: MarvPluginHttpRouteHandler },
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: MarvPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as MarvPluginChannelRegistration).plugin === "object"
        ? (registration as MarvPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: MarvPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: MarvPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: MarvPluginCommandDefinition) => {
