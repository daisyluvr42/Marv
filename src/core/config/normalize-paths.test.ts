  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.marv/logs/marv.log" },
        hooks: {
          path: "~/.marv/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".marv", "logs", "marv.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".marv", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".marv", "telegram.token"),
      );
