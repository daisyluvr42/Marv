...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "marv.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.marv/workspace/canvas). */
  root?: string;
  /** HTTP port to listen on (default: 4246). */
  port?: number;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/marv"). */
  basePath?: string;
...
export type GatewayConfig = {
  /** Single multiplexed port for Gateway WS + HTTP (default: 4242). */
  port?: number;
  /**
