...
import type { ConfigFileSnapshot } from "./types.marv.js";
import { MarvSchema } from "./zod-schema.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.marv/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/marv/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/marv/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/marv/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/marv/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MarvSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MarvConfig";
