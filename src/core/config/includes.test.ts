...
const ETC_MARV_DIR = path.join(ROOT_DIR, "etc", "marv");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "marv.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMarvPath(...parts: string[]) {
  return path.join(ETC_MARV_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcMarvPath("agents.json");
  ...
  it("rejects parent directory traversal escaping config directory (CWE-22)", () => {
    ...
    expect(() => resolve(obj, files, configPath("sub", "marv.json"))).toThrow(ConfigIncludeError);
    expect(() => resolve(obj, files, configPath("sub", "marv.json"))).toThrow(
  ...
  it("supports modular config structure", () => {
    const files = {
      [configPath("gateway.json")]: {
        gateway: { port: 4242, bind: "loopback" },
      },
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marv-includes-symlink-"));
      try {
