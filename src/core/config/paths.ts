...
const NEW_STATE_DIRNAME = ".marv";
const CONFIG_FILENAME = "marv.json";
...
/** Build a homedir thunk that respects MARV_HOME for the given env. */
...
export const DEFAULT_GATEWAY_PORT = 4242;

/**
 * Gateway lock directory (ephemeral).
 * Default: os.tmpdir()/marv-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `marv-${uid}` : "marv";
...
export function resolveGatewayPort(cfg?: MarvConfig, env: NodeJS.ProcessEnv = process.env): number {
  const envRaw =
