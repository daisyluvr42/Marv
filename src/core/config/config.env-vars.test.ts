...
describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as MarvConfig);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as MarvConfig);
      expect(process.env.GROQ_API_KEY).toBe("gsk-config");
    });
  });

  it("loads ${VAR} substitutions from ~/.marv/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.MARV_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected MARV_STATE_DIR to be set by withTempHome");
        }
        ...
        const first = resolveConfigEnvVars(config, process.env) as MarvConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as MarvConfig;
