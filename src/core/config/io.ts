...
import type { MarvConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareMarvVersions } from "./version.js";
...
function coerceConfig(value: unknown): MarvConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MarvConfig;
}
...
function stampConfigVersion(cfg: MarvConfig): MarvConfig {
...
function warnIfConfigFromFuture(cfg: MarvConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMarvVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Marv (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MarvConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MarvConfig {
    try {
  ...
  async function writeConfigFile(cfg: MarvConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    try {
      if (deps.fs.existsSync(configPath)) {
        ...
        if (parsedRes.ok) {
          ...
          cfgToWrite = restoreEnvVarRefs(cfgToWrite, parsedRes.parsed, envForRestore) as MarvConfig;
        }
      }
    } catch {
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MarvConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MARV_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MARV_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MARV_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MARV_WATCH_SESSION === "string" &&
        deps.env.MARV_WATCH_SESSION.trim().length > 0
          ? deps.env.MARV_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MARV_WATCH_COMMAND === "string" &&
        deps.env.MARV_WATCH_COMMAND.trim().length > 0
          ? deps.env.MARV_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MARV_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MARV_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MARV_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): MarvConfig {
