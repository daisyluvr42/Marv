...
async function writeConfigForTest(home: string, config: unknown): Promise<void> {
  ...
  await fs.writeFile(path.join(configDir, "marv.json"), JSON.stringify(config, null, 2), "utf-8");
}
...
describe("config pruning defaults", () => {
  ...
  it("enables cache-ttl pruning + 1h cache TTL for Anthropic API keys", async () => {
    await withTempHome(async (home) => {
      ...
      expect(
        (cfg.models?.metadata?.["anthropic/claude-opus-4-5"] as Record<string, unknown> | undefined)
          ?.params,
      ).toEqual(expect.objectContaining({ cacheRetention: "short" }));
    });
  });
