...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marv-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-marv",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-marv#general");
  });
  ...
  it("derives session transcripts dir from MARV_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MARV_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.MARV_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.MARV_STATE_DIR;
    const stateDir = path.resolve("/home/user/.marv");
  ...
  it("resolves cross-agent paths when MARV_STATE_DIR differs from stored paths", () => {
    const prev = process.env.MARV_STATE_DIR;
    process.env.MARV_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.MARV_STATE_DIR;
    process.env.MARV_STATE_DIR = path.resolve("/home/user/.marv");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
