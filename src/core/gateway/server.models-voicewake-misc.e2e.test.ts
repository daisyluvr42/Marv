...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.MARV_STATE_DIR = path.join(homeDir, ".marv");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["marv", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".marv", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["MARV_CANVAS_HOST_PORT", "MARV_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.MARV_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.MARV_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing MARV_CONFIG_PATH");
    }
