...
vi.mock("../../infra/heartbeat/heartbeat-runner.js", () => ({
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.MARV_SKIP_CHANNELS;
    prevSkipGmail = process.env.MARV_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.MARV_SKIP_PROVIDERS;
    process.env.MARV_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.MARV_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.MARV_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MARV_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.MARV_SKIP_PROVIDERS;
    } else {
      process.env.MARV_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
