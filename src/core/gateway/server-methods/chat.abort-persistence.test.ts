...
describe("chat abort transcript persistence", () => {
  it("persists run-scoped abort partial with rpc metadata and idempotency", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("marv-chat-abort-run-");
  ...
  it("persists session-scoped abort partials with rpc metadata", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("marv-chat-abort-session-");
    const respond = vi.fn();
    const context = createChatAbortContext({
      chatAbortControllers: new Map([
  ...
  it("persists /stop partials with stop-command metadata", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("marv-chat-stop-");
    const respond = vi.fn();
    const context = createChatAbortContext({
