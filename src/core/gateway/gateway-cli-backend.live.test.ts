...
const CLI_LIVE = isTruthyEnvValue(process.env.MARV_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.MARV_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.MARV_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("MARV_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.MARV_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `MARV_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliClearEnv =
      parseJsonStringArray(
        "MARV_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.MARV_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.MARV_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    ...
    const disableMcpConfig = process.env.MARV_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
