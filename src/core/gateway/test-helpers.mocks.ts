...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `marv-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.MARV_CONFIG_PATH = path.join(root, "marv.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "marv.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "marv-gateway-test"),
        ...fileDefaults,
...
vi.mock("../../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadMarvPlugins: () => pluginRegistryState.registry,
  };
});
