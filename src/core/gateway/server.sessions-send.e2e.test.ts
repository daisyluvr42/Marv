...
beforeAll(async () => {
  envSnapshot = captureEnv(["MARV_GATEWAY_PORT", "MARV_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.MARV_GATEWAY_PORT = String(gatewayPort);
  process.env.MARV_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createMarvTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("MARV_CONFIG_PATH missing in gateway test environment");
    }
