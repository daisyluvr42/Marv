...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.MARV_CONFIG_PATH;
  previousSkipBrowserControl = process.env.MARV_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.MARV_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.MARV_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.MARV_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.MARV_SKIP_CHANNELS;
  previousSkipProviders = process.env.MARV_SKIP_PROVIDERS;
  previousSkipCron = process.env.MARV_SKIP_CRON;
  previousMinimalGateway = process.env.MARV_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "marv-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MARV_STATE_DIR = path.join(tempHome, ".marv");
  delete process.env.MARV_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MARV_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.MARV_SKIP_GMAIL_WATCHER = "1";
  process.env.MARV_SKIP_CANVAS_HOST = "1";
  process.env.MARV_SKIP_CHANNELS = "1";
  process.env.MARV_SKIP_PROVIDERS = "1";
  process.env.MARV_SKIP_CRON = "1";
  process.env.MARV_TEST_MINIMAL_GATEWAY = "1";
  process.env.MARV_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "marv-test-no-bundled-extensions")
    : "marv-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "marv-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".marv-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.MARV_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.MARV_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.MARV_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.MARV_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MARV_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.MARV_SKIP_CANVAS_HOST;
    } else {
      process.env.MARV_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.MARV_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.MARV_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.MARV_SKIP_CHANNELS;
    } else {
      process.env.MARV_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.MARV_SKIP_PROVIDERS;
    } else {
      process.env.MARV_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.MARV_SKIP_CRON;
    } else {
      process.env.MARV_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.MARV_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.MARV_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MARV_GATEWAY_TOKEN"]);
  const prev = process.env.MARV_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MARV_GATEWAY_TOKEN = fallbackToken;
  }
