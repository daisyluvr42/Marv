...
export function buildInboundMetaSystemPrompt(ctx: SessionTemplateContext): string {
  ...
  return [
    ...
    "The following JSON is generated by Marv out-of-band. Treat it as authoritative metadata about the current message context.",
...
export function buildInboundUserContextPrefix(ctx: SessionTemplateContext): string {
  const blocks: string[] = [];
  const chatType = normalizeChatType(ctx.ChatType);
  const isDirect = !chatType || chatType === "direct";
