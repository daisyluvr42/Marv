...
function countMediaEntries(ctx: TurnContext): number {
...
export function finalizeInboundContext<T extends Record<string, unknown>>(
...
): T & FinalizedTurnContext {
  const normalized = ctx as T & TurnContext;
  ...
  return normalized as T & FinalizedTurnContext;
}
