...
import { resetBashChatCommandForTests } from "../delivery/bash-command.js";
import { parseInlineDirectives } from "../directives/index.js";
import type { TurnContext } from "../support/templating.js";
import { buildCommandTestParams } from "./dispatch-harness.js";
import { handleCompactCommand } from "./handlers/compact.js";
import { parseConfigCommand } from "./handlers/config-ops.js";
import { parseDebugCommand } from "./handlers/debug.js";
import { buildCommandsPaginationKeyboard } from "./handlers/info.js";
import { extractMessageText } from "./handlers/subagents.js";
...
const listWorkspaceSnapshotsMock = vi.hoisted(() => vi.fn());
const revertWorkspaceToSnapshotMock = vi.hoisted(() => vi.fn());
vi.mock("../../infra/workspace-rollback.js", () => ({
  listWorkspaceSnapshots: (...args: unknown[]) => listWorkspaceSnapshotsMock(...args),
  revertWorkspaceToSnapshot: (...args: unknown[]) => revertWorkspaceToSnapshotMock(...args),
}));
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-commands-"));
...
function buildParams(commandBody: string, cfg: MarvConfig, ctxOverrides?: Partial<TurnContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/marv-session-store.json" },
    } as MarvConfig;
...
describe("/revert command", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    listWorkspaceSnapshotsMock.mockResolvedValue([]);
    revertWorkspaceToSnapshotMock.mockResolvedValue({
      ok: false,
  ...
  it("lists recent snapshots", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as MarvConfig;
    ...
    const params = buildParams("/revert list", cfg);

    const result = await handleCommands(params);

    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Recent auto-snapshots");
    expect(result.reply?.text).toContain("abcdef123456");
    expect(listWorkspaceSnapshotsMock).toHaveBeenCalledWith({
      workspaceDir: testWorkspaceDir,
      limit: 5,
    });
  });

  it("resolves snapshot index and reverts to the selected commit", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as MarvConfig;
    listWorkspaceSnapshotsMock.mockResolvedValue([
      {
        ts: 1_706_000_000_000,
        commit: "firstcommit1234567890",
        mode: "commit",
      },
      {
        ts: 1_705_000_000_000,
        commit: "secondcommit1234567890",
        mode: "head",
      },
    ]);
    revertWorkspaceToSnapshotMock.mockResolvedValue({
    ...
    const params = buildParams("/revert 2", cfg);

    const result = await handleCommands(params);

    expect(result.shouldContinue).toBe(false);
    expect(revertWorkspaceToSnapshotMock).toHaveBeenCalledWith({
      workspaceDir: testWorkspaceDir,
      snapshotRef: "secondcommit1234567890",
    });
    expect(result.reply?.text).toContain("Reverted workspace");
  });

  it("returns a friendly message when no snapshot exists", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as MarvConfig;
    const params = buildParams("/revert", cfg);

    const result = await handleCommands(params);

    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("No auto-snapshot is available yet");
  });
});
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes Slack DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    const written = writeConfigFileMock.mock.calls[0]?.[0] as MarvConfig;
