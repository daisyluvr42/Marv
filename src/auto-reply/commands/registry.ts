...
import { renderKnownCommandArgs } from "./args.js";
import { getChatCommands, getNativeCommandSurfaces } from "./registry-data.js";
import type {
...
export function isCommandEnabled(cfg: MarvConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MarvConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MarvConfig),
...
export function getCommandDetection(_cfg?: MarvConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MarvConfig) {
