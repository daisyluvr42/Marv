...
import { parseActivationCommand } from "../../inbound/group-activation.js";
import { parseSendPolicyCommand } from "../../inbound/send-policy.js";
import { clearSessionQueues } from "../../queue/index.js";
import {
...
} from "../../support/abort.js";
import { normalizeUsageDisplay, resolveResponseUsageMode } from "../../support/thinking.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Marv in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMarvRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Marv via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
...
export function registerSessionCommands(registry: CommandRegistry): void {
  registry.register("activation", handleActivationCommand);
  registry.register("send", handleSendPolicyCommand);
  registry.register("usage", handleUsageCommand);
  registry.register("restart", handleRestartCommand);
  registry.register("stop", handleStopCommand);
  registry.register("__abort_trigger", handleAbortTrigger);
}
