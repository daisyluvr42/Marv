...
} from "./stage-sandbox-media-harness.js";

const sandboxMocks = vi.hoisted(() => ({
  ensureSandboxWorkspaceForSession: vi.fn(),
}));

vi.mock("../../agents/sandbox/sandbox.js", () => sandboxMocks);
...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("marv-triggers-", async (home) => {
      const inboundDir = join(home, ".marv", "media", "inbound");
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("marv-triggers-bypass-", async (home) => {
      // Sensitive host file outside .marv
