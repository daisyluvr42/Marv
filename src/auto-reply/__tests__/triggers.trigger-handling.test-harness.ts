...
vi.mock("../../channels/web/session.js", () => webSessionMocks);
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    { prefix: "marv-triggers-" },
  );
}

export function makeCfg(home: string): MarvConfig {
  return {
    agents: {
      defaults: {
...
export async function loadGetReplyFromConfig() {
  return (await import("../index.js")).getReplyFromConfig;
}
