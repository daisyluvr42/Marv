...
type HomeEnvSnapshot = {
  ...
  MARV_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    MARV_AGENT_DIR: process.env.MARV_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".marv", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MARV_STATE_DIR = path.join(home, ".marv");
    process.env.MARV_AGENT_DIR = path.join(home, ".marv", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".marv", "agent");
