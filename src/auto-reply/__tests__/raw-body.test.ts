...
import { createTempHomeHarness, makeReplyConfig } from "./reply-harness.js";
...
const { withTempHome } = createTempHomeHarness({ prefix: "marv-rawbody-" });
  ...
  it("handles directives and history in the prompt", async () => {
    await withTempHome(async (home) => {
      ...
      const res = await getReplyFromConfig(
        groupMessageCtx,
        {},
        makeReplyConfig(home) as MarvConfig,
      );
