...
import { loadSessionStore, resolveSessionKey } from "../../core/config/sessions.js";
import {
  ...
  makeCfg,
  withTempHome,
} from "./triggers.trigger-handling.test-harness.js";

let getReplyFromConfig: typeof import("../index.js").getReplyFromConfig;
beforeAll(async () => {
  ({ getReplyFromConfig } = await import("../index.js"));
});
...
describe("trigger handling", () => {
  it("runs /compact as a gated command", async () => {
    await withTempHome(async (home) => {
      const storePath = join(tmpdir(), `marv-session-test-${Date.now()}.json`);
