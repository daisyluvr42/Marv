...
import { buildTestCtx } from "./__tests__/test-ctx.js";
import type { ReplyDispatcher } from "./delivery/dispatcher.js";
...
const emptyConfig = {} as MarvConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("does not route when Provider matches OriginatingChannel (even if Surface is missing)", async () => {
    ...
    const replyResolver = async (_ctx: TurnContext, _opts?: GetReplyOptions, _cfg?: MarvConfig) =>
  ...
  it("routes media-only tool results when summaries are suppressed", async () => {
    ...
    const replyResolver = async (_ctx: TurnContext, opts?: GetReplyOptions, _cfg?: MarvConfig) => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as MarvConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
