...
import { enqueueFollowupRun, getFollowupQueueDepth, type FollowupRun } from "../queue/index.js";
...
describe("abort detection", () => {
  ...
  it("triggerBodyNormalized extracts /stop from RawBody for abort detection", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marv-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop")).toBe(true);
    expect(isAbortRequestText("/stop@marv_bot", { botUsername: "marv_bot" })).toBe(true);
  ...
  it("fast-aborts even when text commands are disabled", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marv-abort-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath }, commands: { text: false } } as MarvConfig;
