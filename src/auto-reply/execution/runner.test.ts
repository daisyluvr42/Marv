...
import * as sessions from "../../core/config/sessions.js";
import type { TypingMode } from "../../core/config/types.js";
import { createMockTypingController } from "../__tests__/test-helpers.js";
...
let runReplyAgentPromise: Promise<(typeof import("./runner.js"))["runReplyAgent"]> | undefined;

async function getRunReplyAgent() {
  if (!runReplyAgentPromise) {
    runReplyAgentPromise = import("./runner.js").then((m) => m.runReplyAgent);
  }
...
vi.mock("../../agents/gateway.js", async () => {
  ...
  const gateway = {
    runner: {
      runEmbedded: (params: unknown) => state.runEmbeddedPiAgentMock(params),
      runCli: (params: unknown) => state.runCliAgentMock(params),
    ...
    errors: { ...piHelpers },
    models: {
      isCliProvider: modelSel.isCliProvider,
    ...
    utils: {
      ...
      hasNonzeroUsage: usageModule.hasNonzeroUsage,
    },
  };
...
function createMinimalRun(params?: {
  ...
  const sessionCtx = {
    Provider: "whatsapp",
    MessageSid: "msg",
  } as unknown as SessionTemplateContext;
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  type StateEnvSnapshot = {
    MARV_STATE_DIR: string | undefined;
  };

  function snapshotStateEnv(): StateEnvSnapshot {
    return { MARV_STATE_DIR: process.env.MARV_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.MARV_STATE_DIR === undefined) {
      delete process.env.MARV_STATE_DIR;
    } else {
      process.env.MARV_STATE_DIR = snapshot.MARV_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "marv-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "marv-memory-flush-"));
  });
  ...
  it("continues the foreground reply when a memory flush turn is blocked", async () => {
    await withTempStore(async (storePath) => {
      const sessionKey = "main";
      const sessionEntry = {
        sessionId: "session",
      ...
      state.runEmbeddedPiAgentMock.mockImplementation(async (params: EmbeddedRunParams) => {
        calls.push({ prompt: params.prompt });
        if (params.prompt?.includes("Pre-compaction memory flush.")) {
          throw new Error("CRITICAL: memory_write repeated identical no-progress outcomes");
        }
        return {
          payloads: [{ text: "ok" }],
          meta: { agentMeta: { usage: { input: 1, output: 1 } } },
