...
const {
  models: { lookupContextTokens },
  utils: { resolveCronStyleNow },
  constants: { DEFAULT_CONTEXT_TOKENS, DEFAULT_PI_COMPACTION_RESERVE_TOKENS_FLOOR },
} = agents;
import type { MarvConfig } from "../../core/config/config.js";
import { resolveFreshSessionTotalTokens, type SessionEntry } from "../../core/config/sessions.js";
...
export const DEFAULT_MEMORY_FLUSH_PROMPT = [
  ...
  "Store durable memories now using memory_write (scopeType=agent, kind=session_summary; include date YYYY-MM-DD in content when useful).",
  "Do not edit MEMORY.md / memory/*.md for this flush path.",
  `If nothing to store, reply with ${SILENT_REPLY_TOKEN}.`,
].join(" ");

export const DEFAULT_MEMORY_FLUSH_SYSTEM_PROMPT = [
  "Pre-compaction memory flush turn.",
  "The session is near auto-compaction; capture durable memories to structured memory via memory_write.",
...
export function resolveMemoryFlushSettings(cfg?: MarvConfig): MemoryFlushSettings | null {
