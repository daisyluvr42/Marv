...
import { loadSessionStore, saveSessionStore } from "../../core/config/sessions.js";
...
vi.mock("../../agents/gateway.js", async () => {
  ...
  const gateway = {
    runner: {
      runEmbedded: (params: unknown) => runEmbeddedPiAgentMock(params),
      runCli: (params: unknown) => runCliAgentMock(params),
    ...
    errors: {
      ...(await vi.importActual<typeof import("../../agents/runner/pi-embedded-helpers.js")>(
...
vi.mock("../queue/index.js", async () => {
  const actual = await vi.importActual<typeof import("../queue/index.js")>("../queue/index.js");
  return {
    ...actual,
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("updates totalTokens after auto-compaction using lastCallUsage", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marv-compact-tokens-"));
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marv-usage-last-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
...
describe("runReplyAgent messaging tool suppression", () => {
  ...
  it("persists usage fields even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "marv-session-store-")),
      "sessions.json",
    );
    const sessionKey = "main";
