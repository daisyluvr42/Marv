...
import { createBlockReplyCoalescer } from "./block-coalescer.js";
...
export function createBlockReplyPayloadKey(payload: ReplyPayload): string {
  ...
  // Build a key from the fields that matter for dedup, using a separator
  // that is unlikely to appear in text/URLs. Avoids JSON.stringify overhead.
  const sep = "\x00";
  if (payload.mediaUrls?.length) {
    return `${text}${sep}${payload.mediaUrls.join(sep)}${sep}${replyToId}`;
  }
  if (payload.mediaUrl) {
    return `${text}${sep}${payload.mediaUrl}${sep}${replyToId}`;
  }
  return `${text}${sep}${sep}${replyToId}`;
}
