...
import { parseLineDirectives, hasLineDirectives } from "../directives/line-directives.js";
import { finalizeInboundContext } from "../inbound/context.js";
import { buildInboundUserContextPrefix } from "../inbound/meta.js";
import { normalizeInboundTextNewlines } from "../inbound/text.js";
import type { FollowupRun, QueueSettings } from "../queue/index.js";
import { enqueueFollowupRun, scheduleFollowupDrain } from "../queue/index.js";
import type { TurnContext, SessionTemplateContext } from "../support/templating.js";
import { HEARTBEAT_TOKEN, SILENT_REPLY_TOKEN } from "../support/tokens.js";
import { createReplyDispatcher } from "./dispatcher.js";
import { createReplyToModeFilter, resolveReplyToMode } from "./threading.js";

describe("buildInboundUserContextPrefix", () => {
  it("omits conversation label block for direct chats", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "direct",
      ConversationLabel: "marv-tui",
    } as SessionTemplateContext);
...
describe("inbound context contract (providers + extensions)", () => {
  const cases: Array<{ name: string; ctx: TurnContext }> = [
    {
      name: "whatsapp group",
      ctx: {
        Provider: "whatsapp",
...
describe("createReplyDispatcher", () => {
  ...
  it("preserves intra-lane ordering (priority lane and block lane independently)", async () => {
    const delivered: string[] = [];
    const deliver = vi.fn(async (payload, info) => {
      delivered.push(`${info.kind}:${payload.text}`);
    });
    const dispatcher = createReplyDispatcher({ deliver });

    // Priority lane: tool → final (in order)
    dispatcher.sendToolResult({ text: "t1" });
    dispatcher.sendToolResult({ text: "t2" });
    dispatcher.sendFinalReply({ text: "f1" });

    // Block lane: block → block (in order)
    dispatcher.sendBlockReply({ text: "b1" });
    dispatcher.sendBlockReply({ text: "b2" });

    await dispatcher.waitForIdle();

    // Priority lane items preserve their order
    const priorityItems = delivered.filter((d) => d.startsWith("tool:") || d.startsWith("final:"));
    expect(priorityItems).toEqual(["tool:t1", "tool:t2", "final:f1"]);

    // Block lane items preserve their order
    const blockItems = delivered.filter((d) => d.startsWith("block:"));
    expect(blockItems).toEqual(["block:b1", "block:b2"]);
  ...
  it("tool and final replies are not blocked by block reply human delay", async () => {
    vi.useFakeTimers();
    const randomSpy = vi.spyOn(Math, "random").mockReturnValue(0);
    ...
    // Send first block (no delay), then second block (800ms delay).
    dispatcher.sendBlockReply({ text: "b1" });
    dispatcher.sendBlockReply({ text: "b2" });
    // Send tool + final on priority lane — should NOT wait for block delay.
    dispatcher.sendToolResult({ text: "tool" });
    dispatcher.sendFinalReply({ text: "final" });

    // Flush microtasks so all non-delayed deliveries resolve.
    await vi.advanceTimersByTimeAsync(0);

    // Priority lane (tool + final) delivered immediately; first block also delivered.
    // Second block is still waiting for 800ms human delay.
    expect(delivered.filter((d) => d.kind === "tool")).toHaveLength(1);
    expect(delivered.filter((d) => d.kind === "final")).toHaveLength(1);
    expect(delivered.filter((d) => d.kind === "block")).toHaveLength(1);
    expect(delivered).toHaveLength(3);

    // Advance past the human delay — second block delivers.
    await vi.advanceTimersByTimeAsync(800);
    await dispatcher.waitForIdle();
    expect(delivered.filter((d) => d.kind === "block")).toHaveLength(2);
