...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MARV_HOME for tilde expansion", () => {
    vi.stubEnv("MARV_HOME", "/srv/marv-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/marv-home", "cron", "jobs.json"));
  });
});
