...
async function main() {
  ...
  const { ensureMarvCliOnPath } = await import("../infra/path-env.js");
  ensureMarvCliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[marv] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[marv] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
