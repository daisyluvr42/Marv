...
vi.mock("../channels/web/qr-image.js", () => ({
...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { MARV_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MARV_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports MARV_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { MARV_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MARV_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { MARV_SMOKE: "nope" })).toBe("qr");
  });
});
...
describe("runRelaySmokeTest", () => {
  it("runs qr smoke test", async () => {
    ...
    const mod = await import("../channels/web/qr-image.js");
