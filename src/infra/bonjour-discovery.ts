...
const GATEWAY_SERVICE_TYPE = "_marvbot._tcp";
const GATEWAY_SERVICE_TYPES = [GATEWAY_SERVICE_TYPE] as const;
...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    const line = raw.trim();
    if (!line || !GATEWAY_SERVICE_TYPES.some((serviceType) => line.includes(serviceType))) {
      continue;
    }
    if (!line.includes("Add")) {
      continue;
    }
    const match = line.match(/_marvbot\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
async function discoverWideAreaViaTailnetDns(
  ...
  const probeNames = GATEWAY_SERVICE_TYPES.map(
    (serviceType) => `${serviceType}.${domain.replace(/\.$/, "")}`,
  );
  ...
  for (const ptr of ptrs) {
    ...
    const instanceName = ptrName.replace(/\.?_marvbot\._tcp\..*$/, "");
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (
      line.startsWith("=") &&
      GATEWAY_SERVICE_TYPES.some((serviceType) => line.includes(serviceType))
    ) {
      if (current) {
        results.push(current);
      }
      const matchedServiceType = GATEWAY_SERVICE_TYPES.find((serviceType) =>
        line.includes(serviceType),
      );
      const marker = ` ${matchedServiceType ?? GATEWAY_SERVICE_TYPE}`;
...
async function discoverViaAvahi(
  ...
  for (const serviceType of GATEWAY_SERVICE_TYPES) {
    ...
    results.push(...parseAvahiBrowse(browse.stdout));
  }
  return results.map((beacon) => ({
    ...beacon,
    domain,
  }));
}
