...
let ensureMarvCliOnPath: typeof import("./path-env.js").ensureMarvCliOnPath;

describe("ensureMarvCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MARV_PATH_BOOTSTRAPPED",
    "MARV_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMarvCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling marv exists", () => {
    const tmp = abs("/tmp/marv-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "marv");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MARV_PATH_BOOTSTRAPPED;
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/marv-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "marv");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/marv-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "marv");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "marv");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/marv-path/case-linuxbrew");
