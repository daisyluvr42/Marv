...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "marv";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "marv");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "marv-gw");
  ...
  return `; marv-content-hash: ${contentHash}\n${contentBody}`;
}
