...
const EXTRA_MARKERS = ["marv", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["marv_service_marker"];
  const kindKeys = ["marv_service_kind"];
...
function isMarvGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.marv.");
}
...
function isMarvGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("marv-gateway")) {
    return false;
  }
...
function isMarvGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("marv gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "marv" || isLegacyLabel(label),
    });
  }
