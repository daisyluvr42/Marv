...
function resolveHeartbeatConfig(cfg: MarvConfig, agentId?: string): HeartbeatConfig | undefined {
...
export function resolveHeartbeatPrompt(cfg: MarvConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MarvConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MarvConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
