...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.MARV_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.gatewayPort).toBe("4242");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("marv (Marv)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("marv");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("marv.local");
