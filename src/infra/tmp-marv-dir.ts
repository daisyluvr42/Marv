...
export function resolvePreferredMarvTmpDir(
  options: ResolvePreferredMarvTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "marv" : `marv-${uid}`;
  ...
  for (const preferredPath of preferredCandidates) {
    try {
      ...
      accessSync(preferredPath, fs.constants.W_OK | fs.constants.X_OK);
  ...
  try {
    ...
    mkdirSync(POSIX_MARV_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_MARV_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
      }
      if (!isSecureDirForUser(preferred)) {
