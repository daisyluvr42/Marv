...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.MARV_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Marv. You can keep running the web gateway without it: \`${formatCliCommand("marv gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
