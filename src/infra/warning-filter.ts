...
export function installProcessWarningFilter(): void {
  const globalState = globalThis as typeof globalThis & {
    ...
    [legacyWarningFilterKey]?: ProcessWarningInstallState;
  };
  if (globalState[warningFilterKey]?.installed || globalState[legacyWarningFilterKey]?.installed) {
    return;
  }
  ...
  globalState[legacyWarningFilterKey] = {
    installed: true,
  };
}
