...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MarvConfig | null = null;
  ...
  get(key: string, cfg: MarvConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MarvConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MarvConfig): void {
