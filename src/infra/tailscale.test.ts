...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.MARV_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.MARV_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.MARV_TEST_TAILSCALE_BINARY;
    } else {
      process.env.MARV_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
