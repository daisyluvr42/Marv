...
vi.mock("./marv-root.js", () => ({
  resolveMarvPackageRoot: vi.fn(async () => null),
  resolveMarvPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveMarvPackageRoot when available", async () => {
    const marvRoot = await import("./marv-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-marv");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/marv-bundle");
    (
      marvRoot.resolveMarvPackageRootSync as unknown as ReturnType<typeof vi.fn>
