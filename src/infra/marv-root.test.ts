...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__marv_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "marv-root");
...
describe("resolveMarvPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("returns null for non-marv package roots", async () => {
    const { resolveMarvPackageRootSync } = await import("./marv-root.js");

    const pkgRoot = fx("not-marv");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-marv" }));

    expect(resolveMarvPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
