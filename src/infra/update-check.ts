...
export async function fetchNpmTagVersion(params: {
  ...
  const packageNames = ["marv", "openclaw"];
  try {
    let lastError: string | null = null;
    for (const pkg of packageNames) {
      const res = await fetchWithTimeout(
        `https://registry.npmjs.org/${pkg}/${encodeURIComponent(tag)}`,
        {},
      ...
      lastError = "missing version in registry response";
    }
    return { tag, version: null, error: lastError ?? "registry lookup failed" };
  } catch (err) {
