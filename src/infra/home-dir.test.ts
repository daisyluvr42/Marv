...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MARV_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MARV_HOME when set to ~", () => {
    const env = {
      MARV_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MARV_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MARV_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MARV_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MARV_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MARV_HOME: "/srv/marv-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/marv-home")}/x`);
  });
