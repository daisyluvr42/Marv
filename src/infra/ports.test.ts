...
describe("ports helpers", () => {
  ...
  it("prints an Marv-specific hint when port details look like another Marv instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js marv gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another Marv instance is already running");
  });
  ...
  it("classifies ssh and gateway listeners", () => {
    ...
    expect(
      classifyPortListener(
        {
          commandLine: "node /Users/me/Projects/marv/dist/entry.js gateway",
        },
        18789,
      ),
    ).toBe("gateway");
