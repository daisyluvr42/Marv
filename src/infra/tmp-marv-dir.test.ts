...
import { POSIX_MARV_TMP_DIR, resolvePreferredMarvTmpDir } from "./tmp-marv-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMarvTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `marv-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMarvTmpDir({
    accessSync,
...
describe("resolvePreferredMarvTmpDir", () => {
  it("prefers /tmp/marv when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MARV_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/marv when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MARV_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/marv when /tmp/marv is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/marv when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/marv is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/marv is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/marv is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
