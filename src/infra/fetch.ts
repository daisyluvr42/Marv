...
const wrapFetchWithAbortSignalMarker = Symbol.for("marv.fetch.abort-signal-wrapped");
const legacyWrapFetchWithAbortSignalMarker = Symbol.for("openclaw.fetch.abort-signal-wrapped");

type FetchWithAbortSignalMarker = typeof fetch & {
  [wrapFetchWithAbortSignalMarker]?: true;
  [legacyWrapFetchWithAbortSignalMarker]?: true;
};
...
export function wrapFetchWithAbortSignal(fetchImpl: typeof fetch): typeof fetch {
  const markerFetch = fetchImpl as FetchWithAbortSignalMarker;
  if (
    markerFetch[wrapFetchWithAbortSignalMarker] ||
    markerFetch[legacyWrapFetchWithAbortSignalMarker]
  ) {
    return fetchImpl;
  }
  ...
  Object.defineProperty(wrappedFetch, legacyWrapFetchWithAbortSignalMarker, {
    value: true,
    enumerable: false,
    configurable: false,
    writable: false,
