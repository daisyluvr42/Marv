...
function candidateBinDirs(opts: EnsureMarvPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `marv` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "marv");
...
/**
 * Best-effort PATH bootstrap so skills that require the `marv` CLI can run
