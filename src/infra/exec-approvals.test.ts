...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "marv-exec-approvals-"));
}
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevMarvHome = process.env.MARV_HOME;
    try {
      process.env.MARV_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".marv", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".marv", "exec-approvals.sock")),
      );
    } finally {
      if (prevMarvHome === undefined) {
        delete process.env.MARV_HOME;
      } else {
        process.env.MARV_HOME = prevMarvHome;
      }
    }
  });
});
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.MARV_HOME = dir;
      const approvalsPath = path.join(dir, ".marv", "exec-approvals.json");
