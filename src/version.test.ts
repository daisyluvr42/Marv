...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({ name: "marv", version: "1.2.3" }),
        "utf-8",
      );
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ name: "marv", version: "2.3.4" }),
        "utf-8",
      );
      await fs.writeFile(
  ...
  it("prefers MARV_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        MARV_VERSION: "9.9.9",
        MARV_SERVICE_VERSION: "2.2.2",
