...
import type { MarvConfig, ConfigValidationIssue } from "./types.js";
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MarvConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MarvSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MarvConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MarvConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MarvConfig,
  };
}
