...
const connectNodeClient = async (params: {
  ...
  const token = process.env.MARV_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("MARV_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".marv", "restart-sentinel.json");
