    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marv nodes status", "List known nodes with live status."],
          ["marv nodes pairing pending", "Show pending node pairing requests."],
          ['marv nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["marv nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.marv.ai/cli/nodes")}\n`,
    );
