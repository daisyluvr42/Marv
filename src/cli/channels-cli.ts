    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marv channels list", "List configured channels and auth profiles."],
          ["marv channels status --probe", "Run channel status checks and probes."],
          [
            "marv channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["marv channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.marv.ai/cli/channels",
        )}\n`,
    );
