...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    MARV_STATE_DIR: process.env.MARV_STATE_DIR,
    MARV_CONFIG_PATH: process.env.MARV_CONFIG_PATH,
    MARV_GATEWAY_PORT: process.env.MARV_GATEWAY_PORT,
    MARV_PROFILE: process.env.MARV_PROFILE,
  };

  beforeEach(() => {
    process.env.MARV_STATE_DIR = "/tmp/marv-cli-state";
    process.env.MARV_CONFIG_PATH = "/tmp/marv-cli-state/marv.json";
    delete process.env.MARV_GATEWAY_PORT;
    delete process.env.MARV_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.MARV_STATE_DIR !== undefined) {
      process.env.MARV_STATE_DIR = originalEnv.MARV_STATE_DIR;
    } else {
      delete process.env.MARV_STATE_DIR;
    }

    if (originalEnv.MARV_CONFIG_PATH !== undefined) {
      process.env.MARV_CONFIG_PATH = originalEnv.MARV_CONFIG_PATH;
    } else {
      delete process.env.MARV_CONFIG_PATH;
    }

    if (originalEnv.MARV_GATEWAY_PORT !== undefined) {
      process.env.MARV_GATEWAY_PORT = originalEnv.MARV_GATEWAY_PORT;
    } else {
      delete process.env.MARV_GATEWAY_PORT;
    }

    if (originalEnv.MARV_PROFILE !== undefined) {
      process.env.MARV_PROFILE = originalEnv.MARV_PROFILE;
    } else {
      delete process.env.MARV_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MARV_PROFILE: "dev",
        MARV_STATE_DIR: "/tmp/marv-daemon-state",
