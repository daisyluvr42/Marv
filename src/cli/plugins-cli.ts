...
function applySlotSelectionForPlugin(
  config: MarvConfig,
  pluginId: string,
): { config: MarvConfig; warnings: string[] } {
...
function enablePluginInConfig(config: MarvConfig, pluginId: string): MarvConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: MarvConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs: /plugin");
      lines.push("");
