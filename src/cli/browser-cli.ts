...
} from "./browser-cli-actions-observe.js";
import {
  BROWSER_DEBUG_COMMAND_POLICIES,
  registerBrowserDebugCommands,
} from "./browser-cli-debug.js";
...
export const BROWSER_CLI_COMMAND_POLICIES = [
  ...BROWSER_MANAGE_COMMAND_POLICIES,
  ...BROWSER_OBSERVE_COMMAND_POLICIES,
  ...BROWSER_DEBUG_COMMAND_POLICIES,
];
...
export function registerBrowserCli(program: Command) {
  const browser = program
    .command("browser")
    .description("Manage Marv's dedicated browser (Chrome/Chromium)")
    ...
    .action(() => {
      ...
      defaultRuntime.error(
        danger(`Missing subcommand. Try: "${formatCliCommand("marv browser status")}"`),
      );
