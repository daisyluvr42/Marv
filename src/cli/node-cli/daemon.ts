...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("marv node install"), formatCliCommand("marv node start")];
...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 4242;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("marv node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MARV_LOG_PREFIX: baseEnv.MARV_LOG_PREFIX ?? "node",
