...
const MARV_REPO_URL = "https://github.com/marv/marv.git";
...
export function resolveGitInstallDir(): string {
  const override = process.env.MARV_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MARV_GIT_DIR points at a non-git directory: ${params.dir}. Set MARV_GIT_DIR to an empty folder or an marv checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MARV_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "marv.mjs");
