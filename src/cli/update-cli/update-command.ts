...
import { installCompletion } from "../completion-utils.js";
...
const UPDATE_QUIPS = [
  ...
  "Fresh code, same robot. Miss me?",
  "Back and better. Did you even notice I was gone?",
  "Update complete. I learned some new tricks while I was out.",
  "Upgraded! Now with 23% more sass.",
  "I've evolved. Try to keep up.",
  "New version, who dis? Oh right, still me but shinier.",
  "Patched, polished, and ready to compute. Let's go.",
  "The robot rebooted. Faster circuits, sharper logic.",
  "Update done! Check the changelog or just trust me, it's good.",
  "Reforged in the fires of npm. Stronger now.",
  ...
  "Reboot complete. Please don't look at my bootloader phase.",
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("marv completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
export async function maybeRestartService(params: {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.MARV_UPDATE_IN_PROGRESS = "1";
        try {
      ...
      if (!params.opts.json && restartInitiated) {
        ...
        defaultRuntime.log(
          theme.muted(
            `Verify with \`${replaceCliName(formatCliCommand("marv gateway status"), CLI_NAME)}\` once the gateway is back.`,
          ),
        );
        defaultRuntime.log("");
      }
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("marv gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("marv doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("marv gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("marv gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Marv..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Marv install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("marv doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("marv gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
