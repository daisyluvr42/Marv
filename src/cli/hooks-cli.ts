...
function buildHooksReport(config: MarvConfig): HookStatusReport {
...
function enableInternalHookEntries(config: MarvConfig, hookNames: string[]): MarvConfig {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("marv hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("marv hooks list")}\` to see available hooks.`;
  }
