...
export const BROWSER_MANAGE_COMMAND_POLICIES = defineCommandPolicies("browser", [
  {
    path: "status",
    cliBootstrap: "skip",
    sideEffect: "none",
...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "marv";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (marv|extension). Default: marv")
    .action(
