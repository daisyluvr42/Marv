...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["MARV_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-bundled-skills-test-"));
    process.env.MARV_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
