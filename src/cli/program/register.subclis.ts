...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.MARV_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.MARV_DISABLE_LAZY_SUBCOMMANDS);
};
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Marv docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  },
  {
    ...
    hasSubcommands: false,
    register: async (program) => {
      // Keep completion lazy to avoid a startup cycle while still letting the bundler
      // emit the chunk for packaged builds.
