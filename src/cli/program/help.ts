...
const EXAMPLES = [
  ["marv models --help", "Show detailed help for the models command."],
  ["marv channels login --verbose", "Link personal WhatsApp Web and show QR + connection logs."],
  [
  ...
  [
    'marv agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'marv message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.marv-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.marv.ai/cli");
