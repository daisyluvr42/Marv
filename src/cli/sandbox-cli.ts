...
const SANDBOX_EXAMPLES = {
  main: [
    ["marv sandbox list", "List all sandbox containers."],
    ["marv sandbox list --browser", "List only browser containers."],
    ["marv sandbox recreate --all", "Recreate all containers."],
    ["marv sandbox recreate --session main", "Recreate a specific session."],
    ["marv sandbox recreate --agent mybot", "Recreate agent containers."],
    ["marv sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["marv sandbox list", "List all sandbox containers."],
    ["marv sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["marv sandbox recreate --all", "Recreate all containers."],
    ["marv sandbox recreate --session main", "Recreate a specific session."],
    ["marv sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["marv sandbox recreate --browser --all", "Recreate only browser containers."],
    ["marv sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["marv sandbox explain", "Show effective sandbox config."],
    ["marv sandbox explain --session agent:main:main", "Explain a specific session."],
    ["marv sandbox explain --agent work", "Explain an agent sandbox."],
    ["marv sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.marv.ai/cli/sandbox")}\n`,
    )
    .action(() => {
