...
describe("nodes camera helpers", () => {
  ...
  it("builds stable temp paths when id provided", () => {
    ...
    expect(p).toBe(path.join("/tmp", "marv-camera-snap-front-id1.jpg"));
  });

  it("writes camera clip payload to temp path", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marv-test-"));
    try {
      ...
      expect(out).toBe(path.join(dir, "marv-camera-clip-front-clip1.mp4"));
...
describe("nodes screen helpers", () => {
  ...
  it("builds screen record temp path", () => {
    ...
    expect(p).toBe(path.join("/tmp", "marv-screen-record-id1.mp4"));
  });
});
