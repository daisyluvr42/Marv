...
export async function loadValidConfigOrThrow(): Promise<MarvConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: MarvConfig }): {
  provider: string;
  model: string;
} {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("marv agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
