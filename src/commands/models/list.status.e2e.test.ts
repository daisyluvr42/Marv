...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMarvAgentDir: vi.fn().mockReturnValue("/tmp/marv-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/marv-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMarvAgentDir: mocks.resolveMarvAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/marv-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMarvAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/marv-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/marv-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/marv-agent-custom");
