...
function resolveGatewayAuthToken(cfg: MarvConfig, env: NodeJS.ProcessEnv): string | undefined {
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Marv gateway next.");
      }
    }
  }
