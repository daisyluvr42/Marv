...
export async function noteSecurityWarnings(cfg: MarvConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("marv security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("marv configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("marv config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("marv doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("marv config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('marv config set session.dmScope "per-channel-peer"') +
