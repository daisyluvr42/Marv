...
function resolveMode(cfg: MarvConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: MarvConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("marv configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("marv config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("marv setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("marv doctor --fix")}" to apply changes.`);
  }
