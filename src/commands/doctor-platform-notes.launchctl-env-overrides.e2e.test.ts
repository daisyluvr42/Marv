...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MARV_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MARV_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MARV_GATEWAY_TOKEN");
    expect(message).not.toContain("MARV_GATEWAY_PASSWORD");
  });
