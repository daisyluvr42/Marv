...
beforeAll(() => {
  envSnapshot = captureEnv(["MARV_PROFILE"]);
...
vi.mock("../infra/marv-root.js", () => ({
  resolveMarvPackageRoot: vi.fn().mockResolvedValue("/tmp/marv"),
}));
...
vi.mock("../infra/update/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/marv/pnpm-lock.yaml",
      markerPath: "/tmp/marv/node_modules/.modules.yaml",
    },
...
vi.mock("../infra/daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.marv.gateway.plist",
    }),
  }),
}));
vi.mock("../infra/daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.marv.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Marv status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("marv status --all") ||
          l.includes("marv --profile isolated status --all") ||
          l.includes("marv status --all") ||
          l.includes("marv --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MARV_GATEWAY_TOKEN = "abcd1234";
    try {
