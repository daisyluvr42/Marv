...
vi.mock("../channels/telegram/update-offset-store.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../channels/telegram/update-offset-store.js")>();
  return {
    ...actual,
...
describe("channels command", () => {
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:marv|marv)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "marv_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@marv_bot/);
  });
