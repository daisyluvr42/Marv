...
export async function noteMemorySearchHealth(cfg: MarvConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    ...
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("marv config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("marv memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`marv auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("marv config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("marv memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("marv auth add --provider openai")}`,
