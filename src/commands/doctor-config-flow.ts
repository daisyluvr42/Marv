...
function stripUnknownConfigKeys(config: MarvConfig): {
  config: MarvConfig;
  removed: string[];
} {
  const parsed = MarvSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: MarvConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: MarvConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: MarvConfig): Promise<{
  config: MarvConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: MarvConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: MarvConfig): {
  config: MarvConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: MarvConfig): {
  config: MarvConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".marv");
  const targetPath = path.join(targetDir, "marv.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("marv doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("marv doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("marv doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("marv doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("marv doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "marv doctor --fix" to remove these keys.');
    }
  }
