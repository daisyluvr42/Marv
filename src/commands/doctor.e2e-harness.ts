...
export const resolveMarvPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
export const resolveGatewayProgramArguments = vi.fn().mockResolvedValue({
  programArguments: ["node", "cli", "gateway", "--port", "4242"],
}) as unknown as MockFn;
...
vi.mock("../channels/telegram/token.js", () => ({
...
beforeEach(() => {
  ...
  resolveMarvPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MARV_STATE_DIR;
  originalUpdateInProgress = process.env.MARV_UPDATE_IN_PROGRESS;
  process.env.MARV_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marv-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.MARV_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MARV_UPDATE_IN_PROGRESS;
  } else {
    process.env.MARV_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
