...
import { loadConfig, readConfigFileSnapshot, resolveGatewayPort } from "../core/config/config.js";
...
export async function statusAllCommand(
  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const overviewRows = [
      ...
      { Item: "Security", Value: `Run: ${formatCliCommand("marv security audit --deep")}` },
      gatewaySelfLine
