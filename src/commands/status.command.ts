...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Marv status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("marv security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("marv security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("marv status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("marv logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("marv status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("marv gateway probe")}`);
  }
}
