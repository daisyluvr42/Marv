...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MARV_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("marv-oauth-");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MARV_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("marv-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
