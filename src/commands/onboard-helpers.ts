...
import { pickPrimaryLanIPv4, isValidIPv4 } from "../core/gateway/net.js";
...
export function summarizeExistingConfig(config: MarvConfig): string {
  const rows: string[] = [];
...
export function printWizardHeader(runtime: RuntimeEnv) {
  const header = getCliBannerArtLines().join("\n");
  runtime.log(header);
}
