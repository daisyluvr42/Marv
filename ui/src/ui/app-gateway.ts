...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as MarvApp);
      void loadAgents(host as unknown as MarvApp);
      void loadNodes(host as unknown as MarvApp, { quiet: true });
      void loadDevices(host as unknown as MarvApp, { quiet: true });
...
function handleGatewayEventUnsafe(host: GatewayHost, evt: GatewayEventFrame) {
  ...
  if (evt.event === "chat") {
    ...
    const state = handleChatEvent(host as unknown as MarvApp, payload);
    if (state === "final" || state === "error" || state === "aborted") {
      ...
      if (runId && host.refreshSessionsAfterChat.has(runId)) {
        ...
        if (state === "final") {
          void loadSessions(host as unknown as MarvApp, {
    ...
    if (state === "final") {
      void loadChatHistory(host as unknown as MarvApp);
    }
    return;
  }
