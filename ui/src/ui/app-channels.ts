...
export async function handleWhatsAppWait(host: MarvApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MarvApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MarvApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MarvApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MarvApp): string {
...
function resolveGatewayHttpAuthHeader(host: MarvApp): string | null {
...
function buildGatewayHttpHeaders(host: MarvApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MarvApp) {
...
export function handleNostrProfileToggleAdvanced(host: MarvApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MarvApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MarvApp) {
  const state = host.nostrProfileFormState;
