...
This is the full reference for the `marv onboard` CLI wizard.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.marv/marv.json` exists, choose **Keep / Modify / Reset**.
    - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
      (or pass `--reset`).
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `marv doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.marv/.env` so launchd can read it.
    ...
    - OAuth credentials live in `~/.marv/credentials/oauth.json`; auth profiles live in `~/.marv/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.marv/credentials/oauth.json` (or `$MARV_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `marv pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `marv health`.
    - Tip: `marv status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
