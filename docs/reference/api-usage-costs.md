...
Marv features that can generate provider usage or paid API calls.
...
- `marv status --usage` and `marv channels list` show provider **usage windows**
...
`marv models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
