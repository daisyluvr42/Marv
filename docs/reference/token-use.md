---
summary: "How Marv builds prompt context and reports token usage + costs"
read_when:
...
Marv tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Marv assembles its own system prompt on every run. It includes:
...
For images, Marv downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Marv session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `marv status --usage` and `marv channels list` show
...
`cacheWrite`. If pricing is missing, Marv shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Marv can
...
Anthropic's 1M context window is currently beta-gated. Marv can inject the
