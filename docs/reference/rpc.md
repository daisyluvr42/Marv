...
Marv integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Marv owns lifecycle when `channels.signal.autoStart=true`.
...
- Marv spawns `imsg rpc` as a child process (legacy iMessage integration).
