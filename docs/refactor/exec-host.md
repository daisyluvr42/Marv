...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.marv/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.marv/node.json` (node id + pairing token).
...
- Unix socket at `~/.marv/exec-approvals.sock` (0600).
