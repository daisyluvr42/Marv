...
Run the ACP (Agent Client Protocol) bridge that talks to a Marv Gateway.
...
marv acp --session agent:main:main --reset-session
```
...
it to drive a Marv Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `marv acp` over stdio.
...
- `--server <command>`: ACP server command (default: `marv`).
