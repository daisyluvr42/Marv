---
summary: "CLI reference for `marv security` (audit and fix common security footguns)"
read_when:
...
```bash
marv security audit --json | jq '.summary'
marv security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
marv security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
