...
read_when:
  - Designing workspace memory (~/.marv/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Marv integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.marv/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.marv/workspace` as canonical human-readable memory.
...
```
~/.marv/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Marv**, but keep a separable core library.

### Why integrate into Marv?
...
- You want the agent itself to call the tools:
  - `marv memory recall "…" --k 25 --since 30d`
  - `marv memory reflect --since 7d`
...
Pragmatic take for `~/.marv/workspace`:
