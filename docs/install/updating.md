---
summary: "Updating Marv safely (global install or source), plus rollback strategy"
read_when:
  - Updating Marv
...
Marv is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `marv update`, which restarts) → verify.

## Recommended: re-run the website installer (upgrade in place)

The **preferred** update path is to re-run the installer from the website. It
detects existing installs, upgrades in place, and runs `marv doctor` when
needed.
...
- For **source installs**, use:
  ...
  ```bash
  curl -fsSL https://marv.bot/install.sh | bash -s -- --install-method git --no-onboard
  ```
...
- If your Gateway runs as a service, `marv gateway restart` is preferred over killing PIDs.
...
- Installs deps, builds, builds the Control UI, and runs `marv doctor`.
- Restarts the gateway by default (use `--no-restart` to skip).

If you installed via **npm/pnpm** (no git metadata), `marv update` will try to update via your package manager. If it can’t detect the install, use “Update (global install)” instead.

## Update (Control UI / RPC)

The Control UI has **Update & Restart** (RPC: `update.run`). It:

1. Runs the same source-update flow as `marv update` (git checkout only).
...
## Always Run: `marv doctor`

Doctor is the “safe update” command. It’s intentionally boring: repair + migrate + warn.

Note: if you’re on a **source install** (git checkout), `marv doctor` will offer to run `marv update` first.
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current Marv services.
...
- Run `marv doctor` again and read the output carefully (it often tells you the fix).
- Check: [Troubleshooting](/gateway/troubleshooting)
- Ask in Discord: [https://discord.gg/marv](https://discord.gg/marv)
