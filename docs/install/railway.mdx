...
Deploy Marv on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/marv-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/marv` — Control UI

## What you get

- Hosted Marv Gateway + Control UI
...
- `MARV_STATE_DIR=/data/.marv` (recommended)
- `MARV_WORKSPACE_DIR=/data/workspace` (recommended)
- `MARV_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Marv state + workspace so you can migrate to another host without losing config or memory.
