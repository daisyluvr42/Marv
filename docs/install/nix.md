...
The recommended way to run Marv with Nix is via **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** — a batteries-included Home Manager module.
...
Marv reads JSON5 config from `OPENCLAW_CONFIG_PATH` and stores mutable data in `OPENCLAW_STATE_DIR`.
...
- `OPENCLAW_STATE_DIR` (default: `~/.marv`)
- `OPENCLAW_CONFIG_PATH` (default: `$OPENCLAW_STATE_DIR/marv.json`)
