---
summary: "Run Marv Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want Marv running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running Marv in Docker on Hetzner or a similar provider
title: "Hetzner"
---

# Marv on Hetzner (Docker, Production VPS Guide)

## Goal

Run a persistent Marv Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want “Marv 24/7 for ~$5”, this is the simplest reliable setup.
...
```bash
mkdir -p /root/.marv/workspace

# Set ownership to the container user (uid 1000):
chown -R 1000:1000 /root/.marv
```
...
Marv runs in Docker, but Docker is not the source of truth.
...
- Infrastructure: [marv-terraform-hetzner](https://github.com/andreesg/marv-terraform-hetzner)
- Docker config: [marv-docker-config](https://github.com/andreesg/marv-docker-config)
