...
Deploy Marv on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
repository configures everything needed to run Marv:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MARV_STATE_DIR
        value: /data/.marv
      - key: MARV_WORKSPACE_DIR
        value: /data/workspace
      - key: MARV_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/marv`.
...
If you use the original Marv repository, Render will not auto-deploy your Marv. To update it, run a manual Blueprint sync from the dashboard.
...
For Marv, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Marv host.
