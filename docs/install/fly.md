---
title: Fly.io
description: Deploy Marv on Fly.io
---

# Fly.io Deployment

**Goal:** Marv Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-marv

# Create a persistent volume (1GB is usually enough)
fly volumes create marv_data --size 1 --region iad
```
...
| `internal_port = 3000`     | Must match `--port 3000` (or `MARV_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`        | 512MB is too small; 2GB recommended                                     |
| `MARV_STATE_DIR = "/data"` | Persists state on the volume                                            |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set MARV_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `MARV_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `marv.json` where they could be accidentally exposed or logged.
...
**Note:** With `MARV_STATE_DIR=/data`, the config path is `/data/marv.json`.
...
Or visit `https://my-marv.fly.dev/`

Paste your gateway token (the one from `MARV_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `MARV_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/marv.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/marv.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/marv.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/marv.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/marv.json"
```
...
**Fix:** Ensure `MARV_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-marv

# Release public IPs
fly ips release <public-ipv4> -a my-marv
fly ips release <public-ipv6> -a my-marv
...
fly ips allocate-v6 --private -a my-marv
```
