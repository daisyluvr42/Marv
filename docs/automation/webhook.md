...
read_when:
  ...
  - Wiring external systems into Marv
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Marv config directory (typically `~/.marv/hooks/transforms`).
...
- `marv webhooks gmail setup` writes `hooks.gmail` config for `marv webhooks gmail run`.
...
```bash
curl -X POST http://127.0.0.1:4242/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:4242/hooks/agent \
  -H 'x-marv-token: SECRET' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST http://127.0.0.1:4242/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
