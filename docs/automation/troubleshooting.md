...
marv system heartbeat last
marv logs --follow
marv config get agents.defaults.heartbeat
marv channels status --probe
```
...
```bash
marv config get agents.defaults.heartbeat.activeHours
marv config get agents.defaults.heartbeat.activeHours.timezone
marv config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
marv cron list
marv logs --follow
```
