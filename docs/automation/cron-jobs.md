...
marv cron list
marv cron run <job-id>
marv cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.marv/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `marv cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Marv applies a
...
If `delivery` is omitted for isolated jobs, Marv defaults to `announce`.
...
- Job store: `~/.marv/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.marv/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```bash
marv cron run <jobId>
marv cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
marv cron edit <jobId> \
...
```bash
marv system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`marv system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `MARV_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Marv applies exponential retry backoff for recurring jobs after consecutive errors:
