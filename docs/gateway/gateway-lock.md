...
- The gateway binds the WebSocket listener (default `ws://127.0.0.1:4242`) immediately on startup using an exclusive TCP listener.
...
- If the port is occupied by _another_ process, the error is the same; free the port or choose another with `marv gateway --port <port>`.
