...
Marv’s Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `marv agent`), so routing/permissions/config match your Gateway.
...
- `model: "marv:<agentId>"` (example: `"marv:main"`, `"marv:beta"`)
...
```bash
curl -sS http://127.0.0.1:4242/v1/chat/completions \
