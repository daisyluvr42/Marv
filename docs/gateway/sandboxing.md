---
summary: "How Marv sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Marv mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Marv blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
- Use `marv sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
