...
read_when:
  - Setting up Marv for the first time
...
<Tip>
**New to configuration?** Start with `marv onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    marv onboard       # full setup wizard
    marv configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Control UI">
    Open [http://127.0.0.1:4242](http://127.0.0.1:4242) and use the **Config** tab.
...
<Warning>
Marv only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`marv doctor`, `marv logs`, `marv health`, `marv status`)
- Run `marv doctor` to see exact issues
- Run `marv doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `marv config set` for single keys.
    </Warning>
  ...
  <Accordion title="Semantic patch proposals + event ledger">
    Marv also supports semantic config patching with proposal/commit/rollback flow, plus event-sourcing audit records.

    Methods:

    - `config.patches.propose` — convert natural language into a structured config patch proposal
    - `config.patches.commit` — commit an open proposal (writes config + schedules restart)
    - `config.revisions.rollback` — roll back a committed revision
    - `config.revisions.list` — list semantic revision history
    - `ledger.events.query` — query audit events by `conversationId`
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Marv runs your login shell and imports only the missing keys:
