...
read_when:
  - Running Marv behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Marv
...
- You run Marv behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Marv checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Marv extracts the user identity from the configured header
...
`marv security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Marv config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `marv security audit` and review findings
