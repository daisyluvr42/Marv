...
Marv’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`marv agent`), so routing/permissions/config match your Gateway.
...
```bash
curl -sS http://127.0.0.1:4242/v1/responses \
...
```bash
curl -N http://127.0.0.1:4242/v1/responses \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-marv-agent-id: main' \
  -d '{
