...
Marv can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Marv can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Marv defaults
...
TTS config lives under `messages.tts` in `marv.json`.
...
`~/.marv/settings/tts.json`, override with `MARV_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Marv retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Marv registers
