---
summary: "Marv on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Marv on a Raspberry Pi
  - Running Marv on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Marv on Raspberry Pi

## Goal

Run a persistent, always-on Marv Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
ssh -L 4242:localhost:4242 user@gateway-host
...
# Update config
marv config set gateway.bind tailnet
...
Most Marv features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled marv

# Enable if not
sudo systemctl enable marv

# Start on boot
sudo systemctl start marv
```
...
```bash
# Check logs
journalctl -u marv --no-pager -n 100

# Common fix: rebuild
cd ~/marv  # if using hackable install
npm run build
sudo systemctl restart marv
```
