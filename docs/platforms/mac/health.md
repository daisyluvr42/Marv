...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`marv status`, `marv status --deep`, `marv health --json`) and tail `/tmp/marv/marv-*.log` for `web-heartbeat` / `web-reconnect`.
