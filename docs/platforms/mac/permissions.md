...
- Same path: run the app from a fixed location (for Marv, `dist/Marv.app`).
...
```bash
sudo tccutil reset Accessibility ai.marv.mac
sudo tccutil reset ScreenCapture ai.marv.mac
...
Workaround: move files into the Marv workspace (`~/.marv/workspace`) if you want to avoid per-folder grants.
