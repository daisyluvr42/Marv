---
summary: "Marv macOS companion app (menu bar + gateway broker)"
read_when:
...
# Marv macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for Marv. It owns permissions,
...
- Installs the global CLI (`marv`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `marv gateway install`.
...
The app registers the `marv://` URL scheme for local actions.
...
Tip: compare against `marv gateway discover --json` to see whether the
...
- **Local port:** the Gateway port (default `4242`), always stable.
