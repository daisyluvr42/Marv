...
- Set `gateway.bind: "tailnet"` in `~/.openclaw/marv.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `marv.internal.`) on the gateway host and publish `_openclaw-gw._tcp` records.
...
```bash
marv nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__openclaw__/canvas/"}'
```
