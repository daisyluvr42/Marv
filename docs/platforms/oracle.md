---
summary: "Marv on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Marv on Oracle Cloud
  - Looking for low-cost VPS hosting for Marv
  - Want 24/7 Marv on a small server
title: "Oracle Cloud"
---

# Marv on Oracle Cloud (OCI)

## Goal

Run a persistent Marv Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Marv (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname marv
...
sudo tailscale up --ssh --hostname=marv
```

This enables Tailscale SSH, so you can connect via `ssh marv` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@marv` (or use the Tailscale IP).
...
marv doctor --generate-gateway-token
...
```
https://marv.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `marv security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.marv`
- **Security audit:** `marv security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@marv
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=marv --reset
```
