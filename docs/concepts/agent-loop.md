...
In Marv, a loop is a single, serialized run per session that emits lifecycle and stream events
...
4. `subscribeEmbeddedPiSession` bridges pi-agent-core events to Marv `agent` stream:
...
- System prompt is built from Marv’s base prompt, skills prompt, bootstrap context, and per-run overrides.
