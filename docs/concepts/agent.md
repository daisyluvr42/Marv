...
Marv runs a single embedded agent runtime derived from **pi-mono**.
...
Inside `agents.defaults.workspace`, Marv expects these user-editable files:
...
On the first turn of a new session, Marv injects the contents of these files directly into the agent context.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content).

If a file is missing, Marv injects a single “missing file” marker line (and `marv setup` will create a safe default template).
...
Marv reuses pieces of the pi-mono codebase (models/tools), but **session management, discovery, and tool wiring are Marv-owned**.
