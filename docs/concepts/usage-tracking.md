...
- `/usage cost` in chats: local cost summary aggregated from Marv session logs.
- CLI: `marv status --usage` prints a full per-provider breakdown.
- CLI: `marv channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
