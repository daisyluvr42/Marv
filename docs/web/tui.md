...
```bash
marv tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `marv tui --deliver`
...
- Check the Gateway logs: `marv logs --follow`.
- Confirm the agent can run: `marv status` and `marv models status`.
...
- No agents in picker: check `marv agents list` and your routing config.
