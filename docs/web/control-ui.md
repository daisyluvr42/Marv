...
- default: `http://<host>:4242/`
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/marv`)
...
If the page fails to load, start the Gateway first: `marv gateway`.
...
- Config: view/edit `~/.marv/marv.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Marv
...
- `http://<tailscale-ip>:4242/` (or your configured `gateway.controlUi.basePath`)
...
Marv **blocks** Control UI connections without device identity.

**Recommended fix:** use HTTPS (Tailscale Serve) or open the UI locally:

- `https://<magicdns>/` (Serve)
- `http://127.0.0.1:4242/` (on the gateway host)
...
Then point the UI at your Gateway WS URL (e.g. `ws://127.0.0.1:4242`).
