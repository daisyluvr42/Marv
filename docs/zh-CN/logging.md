...
- Marv 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `marv plugins enable diagnostics-otel` 启用插件。
...
- `marv.tokens`（计数器，属性：`marv.token`、`marv.channel`、`marv.provider`、`marv.model`）
- `marv.cost.usd`（计数器，属性：`marv.channel`、`marv.provider`、`marv.model`）
- `marv.run.duration_ms`（直方图，属性：`marv.channel`、`marv.provider`、`marv.model`）
- `marv.context.tokens`（直方图，属性：`marv.context`、`marv.channel`、`marv.provider`、`marv.model`）

消息流：

- `marv.webhook.received`（计数器，属性：`marv.channel`、`marv.webhook`）
- `marv.webhook.error`（计数器，属性：`marv.channel`、`marv.webhook`）
- `marv.webhook.duration_ms`（直方图，属性：`marv.channel`、`marv.webhook`）
- `marv.message.queued`（计数器，属性：`marv.channel`、`marv.source`）
- `marv.message.processed`（计数器，属性：`marv.channel`、`marv.outcome`）
- `marv.message.duration_ms`（直方图，属性：`marv.channel`、`marv.outcome`）

队列 + 会话：

- `marv.queue.lane.enqueue`（计数器，属性：`marv.lane`）
- `marv.queue.lane.dequeue`（计数器，属性：`marv.lane`）
- `marv.queue.depth`（直方图，属性：`marv.lane` 或 `marv.channel=heartbeat`）
- `marv.queue.wait_ms`（直方图，属性：`marv.lane`）
- `marv.session.state`（计数器，属性：`marv.state`、`marv.reason`）
- `marv.session.stuck`（计数器，属性：`marv.state`）
- `marv.session.stuck_age_ms`（直方图，属性：`marv.state`）
- `marv.run.attempt`（计数器，属性：`marv.attempt`）
...
- `marv.webhook.error`
  - `marv.channel`、`marv.webhook`、`marv.chatId`、`marv.error`
- `marv.message.processed`
  - `marv.channel`、`marv.outcome`、`marv.chatId`、`marv.messageId`、`marv.sessionKey`、`marv.sessionId`、`marv.reason`
- `marv.session.stuck`
  - `marv.state`、`marv.ageMs`、`marv.queueDepth`、`marv.sessionKey`、`marv.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `marv doctor`。
