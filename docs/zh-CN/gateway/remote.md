...
- Gateway WebSocket 绑定到你配置端口的 **loopback**（默认为 4242）。
...
- 使用 macOS 应用的 **Remote over SSH** 模式（设置 → 通用 → "Marv runs"）。
...
- `marv health` 和 `marv status --deep` 现在通过 `ws://127.0.0.1:4242` 访问远程 Gateway 网关。
- `marv gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。

注意：将 `4242` 替换为你配置的 `gateway.port`（或 `--port`/`MARV_GATEWAY_PORT`）。
...
当 Gateway 网关仅限 loopback 时，保持 URL 为 `ws://127.0.0.1:4242` 并先打开 SSH 隧道。

## 通过 SSH 的聊天 UI

WebChat 不再使用单独的 HTTP 端口。SwiftUI 聊天 UI 直接连接到 Gateway WebSocket。

- 通过 SSH 转发 `4242`（见上文），然后让客户端连接到 `ws://127.0.0.1:4242`。
