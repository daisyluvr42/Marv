...
read_when:
  - 添加或修改配置字段时
summary: ~/.openclaw/marv.json 的所有配置选项及示例
title: 配置
x-i18n:
...
Marv 从 `~/.openclaw/marv.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，Marv 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.openclaw/workspace`）。通常只在以下情况需要配置：
...
请使用 `config.patch` 或 `marv config set`。请备份 `~/.openclaw/marv.json`。
...
- `~/.openclaw/agent/*`（由 `marv doctor` 迁移到 `~/.openclaw/agents/<defaultAgentId>/agent/*`）
...
- 如需稳定路径，将 `logging.file` 设为 `/tmp/openclaw/marv.log`。
- 控制台输出可通过以下方式单独调整：
...
请先安装：`marv plugins install @openclaw/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
`~/.openclaw/agents/<agentId>/agent/models.json` 或在 Marv 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
...
- Marv.app 监视 `~/.openclaw/marv.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.openclaw/marv.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.openclaw/marv.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `marv --dev …` → 使用 `~/.openclaw-dev` + 端口从基础 `19001` 偏移
- `marv --profile <name> …` → 使用 `~/.openclaw-<name>`（端口通过配置/环境变量/标志）
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `marv`（通告 `marv.local`）。通过 `OPENCLAW_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.openclaw/dns/` 下使用配置的发现域（示例：`marv.internal.`）为 `_openclaw-gw._tcp` 写入单播 DNS-SD 区域。
