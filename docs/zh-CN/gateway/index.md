...
- 替代旧版 `gateway` 命令。CLI 入口点：`marv gateway`。
...
- 配置热重载监视 `~/.marv/marv.json`（或 `MARV_CONFIG_PATH`）。
  - 默认模式：`gateway.reload.mode="hybrid"`（热应用安全更改，关键更改时重启）。
  - 热重载在需要时通过 **SIGUSR1** 使用进程内重启。
  - 使用 `gateway.reload.mode="off"` 禁用。
- 将 WebSocket 控制平面绑定到 `127.0.0.1:<port>`（默认 4242）。
- 同一端口也提供 HTTP 服务（控制界面、hooks、A2UI）。单端口多路复用。
  - OpenAI Chat Completions（HTTP）：[`/v1/chat/completions`](/gateway/openai-http-api)。
  - OpenResponses（HTTP）：[`/v1/responses`](/gateway/openresponses-http-api)。
  - Tools Invoke（HTTP）：[`/tools/invoke`](/gateway/tools-invoke-http-api)。
  - Marv-mem MCP（HTTP）：[`/mcp`](/gateway/mcp-memory-http-api)。
- 默认在 `canvasHost.port`（默认 `4246`）上启动 Canvas 文件服务器，从 `~/.marv/workspace/canvas` 提供 `http://<gateway-host>:4246/__marv__/canvas/`。使用 `canvasHost.enabled=false` 或 `MARV_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway 网关认证：设置 `gateway.auth.token`（或 `MARV_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 loopback 上也是如此。
- 端口优先级：`--port` > `MARV_GATEWAY_PORT` > `gateway.port` > 默认 `4242`。
...
- 首选 Tailscale/VPN；否则使用 SSH 隧道：
  ...
  ```
- 然后客户端通过隧道连接到 `ws://127.0.0.1:4242`。
...
- `MARV_SERVICE_MARKER=marv`
- `MARV_SERVICE_KIND=gateway`
...
- 基础端口 = `gateway.port`（或 `MARV_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础 + 2（仅 loopback）
- `canvasHost.port = 基础 + 4`（或 `MARV_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `MARV_CONFIG_PATH`
- 唯一的 `MARV_STATE_DIR`
...
```bash
marv --profile main gateway install
marv --profile rescue gateway install
```
...
- `health` — 完整健康快照（与 `marv health --json` 形状相同）。
- `status` — 简短摘要。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 Marv 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`marv gateway uninstall`（当前服务）和 `marv doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `marv gateway install --force` 重新安装（配置文件/env/路径更改）。
...
Marv 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`marv gateway install` 写入用户单元。`marv doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/marv-gateway[-<profile>].service`：

```
[Unit]
Description=Marv Gateway (profile: <profile>, v<version>)
...
Restart=always
RestartSec=5
Environment=MARV_GATEWAY_TOKEN=
...
创建 `/etc/systemd/system/marv-gateway[-<profile>].service`（复制上面的单元，
...
- `marv gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `marv message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `marv agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `marv gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `marv gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `marv gateway` 和旧版 TCP 控制端口的使用。
