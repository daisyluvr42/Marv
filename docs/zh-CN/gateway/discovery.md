...
- TXT 键（非机密）：
  ...
  - `sshPort=22`（或广播的端口）
  - `gatewayPort=4242`（Gateway 网关 WS + HTTP）
  - `gatewayTls=1`（仅当启用 TLS 时）
  - `gatewayTlsSha256=<sha256>`（仅当启用 TLS 且指纹可用时）
  - `canvasPort=4246`（默认画布主机端口；服务于 `/__marv__/canvas/`）
