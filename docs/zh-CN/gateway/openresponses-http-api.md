...
Marv 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `marv agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "marv:<agentId>"`（示例：`"marv:main"`、`"marv:beta"`）
