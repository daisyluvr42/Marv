...
summary: 在 exe.dev 上运行 Marv Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：Marv Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `marv devices list` 和 `marv devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
