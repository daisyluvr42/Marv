...
- 默认：`http://<host>:4242/`
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/marv`）
...
- http://127.0.0.1:4242/（或 http://localhost:4242/）

如果页面加载失败，请先启动 Gateway 网关：`marv gateway`。
...
一旦批准，设备会被记住，除非你使用 `marv devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/cli/devices) 了解 token 轮换和撤销。
...
- 配置：查看/编辑 `~/.marv/marv.json`（`config.get`、`config.set`）
...
默认情况下，当 `gateway.auth.allowTailscale` 为 `true` 时，Serve 请求可以通过 Tailscale 身份头（`tailscale-user-login`）进行认证。Marv 通过使用 `tailscale whois` 解析 `x-forwarded-for` 地址并与头匹配来验证身份，并且只在请求通过 Tailscale 的 `x-forwarded-*` 头到达 loopback 时接受这些。如果你想即使对于 Serve 流量也要求 token/密码，请设置 `gateway.auth.allowTailscale: false`（或强制 `gateway.auth.mode: "password"`）。

### 绑定到 tailnet + token

```bash
marv gateway --bind tailnet --token "$(openssl rand -hex 32)"
```

然后打开：

- `http://<tailscale-ip>:4242/`（或你配置的 `gateway.controlUi.basePath`）

将 token 粘贴到 UI 设置中（作为 `connect.params.auth.token` 发送）。

## 不安全的 HTTP

如果你通过普通 HTTP 打开仪表板（`http://<lan-ip>` 或 `http://<tailscale-ip>`），浏览器在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，Marv **阻止**没有设备身份的控制 UI 连接。

**推荐修复：** 使用 HTTPS（Tailscale Serve）或在本地打开 UI：

- `https://<magicdns>/`（Serve）
- `http://127.0.0.1:4242/`（在 Gateway 网关主机上）
...
```bash
MARV_CONTROL_UI_BASE_PATH=/marv/ pnpm ui:build
```
...
然后将 UI 指向你的 Gateway 网关 WS URL（例如 `ws://127.0.0.1:4242`）。
...
```text
http://localhost:5173/?gatewayUrl=ws://<gateway-host>:4242
```

可选的一次性认证（如需要）：

```text
http://localhost:5173/?gatewayUrl=wss://<gateway-host>:4242&token=<gateway-token>
```
