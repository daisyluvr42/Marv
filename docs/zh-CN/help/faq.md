...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 Marv 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-openclaw)
  ...
  - [无法访问 docs.marv.ai（SSL 错误），怎么办？](#i-cant-access-docsopenclawai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 marv](#windows-install-says-git-not-found-or-openclaw-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 Marv？](#how-do-i-install-openclaw-on-linux)
  - [如何在 VPS 上安装 Marv？](#how-do-i-install-openclaw-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 Marv 自行更新吗？](#can-i-ask-openclaw-to-update-itself)
  ...
  - [如果我买了 Mac mini 运行 Marv，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-openclaw-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 Marv 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-openclaw-instances)
  ...
  - [在专用机器上运行 Marv 有多重要？](#how-important-is-it-to-run-openclaw-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 Marv 吗？有什么要求？](#can-i-run-openclaw-in-a-vm-and-what-are-the-requirements)
- [什么是 Marv？](#what-is-openclaw)
  - [用一段话描述 Marv？](#what-is-openclaw-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [Marv 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-openclaw)
  - [Marv 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-openclaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [Marv 能否按计划或在后台持续运行任务？](#can-openclaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [Marv 使用的所有数据都保存在本地吗？](#is-all-data-used-with-openclaw-saved-locally)
  - [Marv 将数据存储在哪里？](#where-does-openclaw-store-its-data)
  ...
  - [如何完全卸载 Marv？](#how-do-i-completely-uninstall-openclaw)
...
- [配置基础](#config-basics)
  ...
  - [Marv 浏览器可以无头运行吗？](#can-the-openclaw-browser-run-headless)
...
- [远程 Gateway 网关与节点](#remote-gateways-nodes)
  ...
  - [两个 Marv 实例（本地 + VPS）可以互相通信吗？](#can-two-openclaw-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [Marv 如何加载环境变量？](#how-does-openclaw-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 Marv 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-openclaw-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 Marv 但保留安装？](#how-do-i-completely-reset-openclaw-but-keep-it-installed)
  ...
  - [为什么 Marv 不在群组中回复？](#why-doesnt-openclaw-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [Marv、Flawd 和 Krill 使用什么模型？](#what-do-openclaw-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `marv gateway status` 显示 `Runtime: running` 但 `RPC probe: failed`？](#why-does-openclaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [为什么 `marv gateway status` 显示 `Config (cli)` 和 `Config (service)` 不同？](#why-does-openclaw-gateway-status-show-config-cli-and-config-service-different)
  - ["another gateway instance is already listening"是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何以远程模式运行 Marv（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 Marv？](#i-closed-my-terminal-on-windows-how-do-i-restart-openclaw)
  ...
  - [通俗解释：`marv gateway restart` 与 `marv gateway`](#eli5-openclaw-gateway-restart-vs-openclaw-gateway)
...
- [安全与访问控制](#security-and-access-control)
  - [将 Marv 暴露给入站私信安全吗？](#is-it-safe-to-expose-openclaw-to-inbound-dms)
...
1. 在新机器上安装 Marv。
2. 从旧机器复制 `$OPENCLAW_STATE_DIR`（默认：`~/.marv`）。
...
### 无法访问 docs.marv.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.marv.ai`。禁用该功能或将 `docs.marv.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
...
（`~/.marv`）和工作区（`~/.openclaw/workspace`）保持不变。
...
使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.openclaw/skills/<name>/SKILL.md`（或通过 `~/.openclaw/marv.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.openclaw/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.openclaw/marv.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.openclaw/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，Marv 将其视为 `<workspace>/skills`。
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 Marv 工作区）；Marv 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.openclaw/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
所有内容位于 `$OPENCLAW_STATE_DIR`（默认：`~/.marv`）下：

| 路径                                                            | 用途                                                 |
| --------------------------------------------------------------- | ---------------------------------------------------- |
| `$OPENCLAW_STATE_DIR/marv.json`                                 | 主配置（JSON5）                                      |
...
旧版单智能体路径：`~/.openclaw/agent/*`（通过 `marv doctor` 迁移）。
...
- **状态目录（`~/.marv`）**：配置、凭据、认证配置文件、会话、日志和共享 Skills（`~/.openclaw/skills`）。
...
Marv 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.openclaw/marv.json`）读取可选的 **JSON5** 配置：
...
- 从备份恢复（git 或复制的 `~/.openclaw/marv.json`）。
...
- 编辑 `~/.openclaw/marv.json` 中的 `agents.defaults.model`
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  ...
  - 旧版：`~/.openclaw/agent/*`（通过 `marv doctor` 迁移）
...
- 每实例使用 `marv --profile <name> …`（自动创建 `~/.openclaw-<name>`）。
- 在每个配置文件配置中设置唯一的 `gateway.port`（或手动运行时传 `--port`）。
- 安装每配置文件的服务：`marv --profile <name> gateway install`。

配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.marv.*`、`openclaw-gateway-<profile>.service`、`Marv Gateway 网关 (<profile>)`）。
