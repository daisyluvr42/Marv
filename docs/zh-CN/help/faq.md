...
- **Tailnet 绑定**：运行 `marv gateway --bind tailnet --token "<token>"`，打开 `http://<tailscale-ip>:18789/`，在仪表板设置中粘贴令牌。
- **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后从 `marv dashboard` 打开 `http://127.0.0.1:18789/?token=...`。
...
```
--port > MARV_GATEWAY_PORT > gateway.port > 默认 18789
```
...
Marv 通过在启动时立即绑定 WebSocket 监听器来强制运行时锁（默认 `ws://127.0.0.1:18789`）。如果绑定因 `EADDRINUSE` 失败，它会抛出 `GatewayLockError` 表示另一个实例已在监听。
