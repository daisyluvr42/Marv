...
Marv 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`marv gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`marv onboard --install-daemon`
- 直接安装：`marv gateway install`
- 配置流程：`marv configure` → 选择 **Gateway service**
- 修复/迁移：`marv doctor`（提供安装或修复服务）
