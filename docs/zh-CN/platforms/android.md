...
- 在 Gateway 网关主机的 `~/.openclaw/marv.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `marv.internal.`）并发布 `_openclaw-gw._tcp` 记录。
...
```bash
marv nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
