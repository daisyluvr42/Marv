...
Android 直接连接到 Gateway 网关 WebSocket（默认 `ws://<host>:4242`）并使用 Gateway 网关拥有的配对。
...
- 你可以在 Gateway 网关机器上运行 CLI（`marv`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.marv/marv.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `marv.internal.`）并发布 `_marv-gw._tcp` 记录。
...
注意：节点使用 `canvasHost.port`（默认 `4246`）上的独立 canvas 主机。

1. 在 Gateway 网关主机上创建 `~/.marv/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
marv nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:4246/__marv__/canvas/"}'
```

Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:4246/__marv__/canvas/`。

此服务器将实时重载客户端注入 HTML 并在文件更改时重新加载。
A2UI 主机位于 `http://<gateway-host>:4246/__marv__/a2ui/`。
