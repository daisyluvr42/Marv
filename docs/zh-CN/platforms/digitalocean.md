---
read_when:
  - 在 DigitalOcean 上设置 Marv
  - 寻找便宜的 VPS 托管来运行 Marv
summary: 在 DigitalOcean 上运行 Marv（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Marv

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Marv Gateway 网关。
...
打开：`http://<tailscale-ip>:4242`（需要令牌）。
