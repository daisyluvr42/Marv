...
Marv.app 不再捆绑 Node/Bun 或 Gateway 网关运行时。macOS 应用期望有一个**外部**的 `marv` CLI 安装，不会将 Gateway 网关作为子进程启动，而是管理一个每用户的 launchd 服务来保持 Gateway 网关运行（或者如果已有本地 Gateway 网关正在运行，则连接到现有的）。

## 安装 CLI（本地模式必需）

你需要在 Mac 上安装 Node 22+，然后全局安装 `marv`：
...
- `ai.marv.gateway`（或 `ai.marv.<profile>`；旧版 `com.marv.*` 可能仍然存在）

Plist 位置（每用户）：

- `~/Library/LaunchAgents/ai.marv.gateway.plist`
  （或 `~/Library/LaunchAgents/ai.marv.<profile>.plist`）

管理者：

- macOS 应用在本地模式下拥有 LaunchAgent 的安装/更新权限。
- CLI 也可以安装它：`marv gateway install`。

行为：

- "Marv Active"启用/禁用 LaunchAgent。
...
```bash
marv gateway call health --url ws://127.0.0.1:18999 --timeout 3000
```
