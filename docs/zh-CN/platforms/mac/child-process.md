...
Gateway 网关；如果无法访问，它会通过外部 `marv` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `ai.marv.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`MARV_PROFILE` 时为 `ai.marv.<profile>`；支持旧版 `com.marv.*`）。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.marv/disable-launchagent`，
