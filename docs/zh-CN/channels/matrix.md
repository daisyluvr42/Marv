...
1. 安装 Matrix 插件：
   - 从 npm：`marv plugins install @openclaw/matrix`
...
3. 获取机器人账户的访问令牌：
   ...
   - 或设置 `channels.matrix.userId` + `channels.matrix.password`：Marv 会调用相同的登录端点，将访问令牌存储在 `~/.openclaw/credentials/matrix/credentials.json`，并在下次启动时重用。
