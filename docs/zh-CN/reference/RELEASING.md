...
- [ ] 确认包元数据（name、description、repository、keywords、license）以及 `bin` 映射指向 [`marv.mjs`](https://github.com/openclaw/openclaw/blob/main/marv.mjs) 作为 `marv`。
...
- [ ] 验证注册表：`npm view marv version`、`npm view marv dist-tags` 和 `npx -y openclaw@X.Y.Z --version`（或 `--help`）。
