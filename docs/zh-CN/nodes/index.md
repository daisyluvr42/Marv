...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `marv nodes …` 可以针对这台 Mac 工作）。
...
marv nodes status
marv nodes describe --node <idOrNameOrIp>
```

注意事项：

- 当节点的设备配对角色包含 `node` 时，`nodes status` 将节点标记为**已配对**。
- `node.pair.*`（CLI：`marv nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.marv/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.marv/marv.json`）。
- `marv node run` 读取 `MARV_GATEWAY_TOKEN` 进行认证。
...
- 在 `marv node run` / `marv node install` 上使用 `--display-name`（持久化在节点上的 `~/.marv/node.json` 中）。
- `marv nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
```bash
marv approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
marv approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

批准存储在节点主机的 `~/.marv/exec-approvals.json` 中。
...
```bash
marv config set tools.exec.host node
marv config set tools.exec.security allowlist
marv config set tools.exec.node "<id-or-name>"
```
...
```bash
marv nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
marv nodes canvas snapshot --node <idOrNameOrIp> --format png
marv nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas 控制

```bash
marv nodes canvas present --node <idOrNameOrIp> --target https://example.com
marv nodes canvas hide --node <idOrNameOrIp>
marv nodes canvas navigate https://example.com --node <idOrNameOrIp>
marv nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
marv nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
marv nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
marv nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
marv nodes camera list --node <idOrNameOrIp>
marv nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
marv nodes camera snap --node <idOrNameOrIp> --facing front
```

视频片段（`mp4`）：

```bash
marv nodes camera clip --node <idOrNameOrIp> --duration 10s
marv nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
marv nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
marv nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
marv nodes location get --node <idOrNameOrIp>
marv nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
marv nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Marv"}'
```
...
```bash
marv nodes run --node <idOrNameOrIp> -- echo "Hello from mac node"
marv nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
```
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.marv/exec-approvals.json`）。
...
Marv 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.marv/node.json` 中。
- Exec 批准通过 `~/.marv/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/tools/exec-approvals)）。
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  如果应用不可用则回退到本地执行。设置 `MARV_NODE_EXEC_HOST=app` 要求
  使用应用，或设置 `MARV_NODE_EXEC_FALLBACK=0` 禁用回退。
- 当 Gateway 网关 WS 使用 TLS 时，添加 `--tls` / `--tls-fingerprint`。

## Mac 节点模式

- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `marv nodes …` 可以针对这台 Mac 工作）。
