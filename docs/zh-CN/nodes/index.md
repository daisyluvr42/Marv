...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.openclaw/marv.json`）。
- `marv node run` 读取 `OPENCLAW_GATEWAY_TOKEN` 进行认证。
...
- 在 `marv node run` / `marv node install` 上使用 `--display-name`（持久化在节点上的 `~/.openclaw/node.json` 中）。
