...
- **个性化设置存放在仓库之外：** `~/.openclaw/workspace`（工作区）+ `~/.openclaw/marv.json`（配置）。
...
- **配置：** `~/.openclaw/marv.json`（JSON/JSON5 格式）
...
- 将 `~/.openclaw/workspace` 和 `~/.openclaw/` 保持为"你的东西"；不要将个人提示/配置放入 `marv` 仓库。
