...
你可以通过 `marv.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用未知或未加载的插件工具名称，Marv 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:marv`：所有内置 Marv 工具（不包括提供商插件）
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`marv node run`）。
...
- 需要 Brave API 密钥（推荐：`marv configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速冒烟测试：`marv nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`marv gateway` 就地重启）
