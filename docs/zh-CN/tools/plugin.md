...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 Marv。

大多数时候，当你想要一个尚未内置到核心 Marv 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
Marv 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
每个插件必须在其根目录中包含 `marv.plugin.json` 文件。如果路径指向文件，则插件根目录是文件的目录，必须包含清单。

如果多个插件解析到相同的 id，上述顺序中的第一个匹配项获胜，较低优先级的副本被忽略。

### 包集合

插件目录可以包含带有 `marv.extensions` 的 `package.json`：
...
渠道插件可以通过 `marv.channel` 广播新手引导元数据，通过 `marv.install` 广播安装提示。这使核心目录保持无数据。
...
如果插件导出 `id`，Marv 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用嵌入在 `marv.plugin.json`（`configSchema`）中的 JSON Schema 进行验证。
...
插件也可以注册自己的顶级命令（例如：`marv voicecall`）。
...
- 插件管理的钩子在 `marv hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `marv hooks` 启用/禁用插件管理的钩子；而是启用/禁用插件。

## 提供商插件（模型认证）

插件可以注册**模型提供商认证**流程，以便用户可以在 Marv 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试）并验证 `marv.extensions` 指向构建的入口点（`dist/index.js`）。
