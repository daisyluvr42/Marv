...
summary: 修复 Linux 上 Marv 浏览器控制的 Chrome/Brave/Edge/Chromium CDP 启动问题
title: 浏览器故障排除
x-i18n:
...
## 问题："Failed to start Chrome CDP on port 4253"
...
在 Ubuntu（和许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制会干扰 Marv 启动和监控浏览器进程的方式。
...
如果你必须使用 snap Chromium，配置 Marv 附加到手动启动的浏览器：
...
| `browser.cdpPort`        | Chrome DevTools Protocol 端口                                 | `4253`                                           |

### 问题："Chrome extension relay is running, but no tab is connected"

你正在使用 `chrome` 配置文件（扩展中继）。它期望 Marv 浏览器扩展附加到一个活动标签页。
...
- 本地 `marv` 配置文件自动分配 `cdpPort`/`cdpUrl`；仅为远程 CDP 设置这些。
