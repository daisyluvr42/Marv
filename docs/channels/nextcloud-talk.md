...
```bash
marv plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
marv plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Marv" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `marv pairing list nextcloud-talk`
  - `marv pairing approve nextcloud-talk <CODE>`
