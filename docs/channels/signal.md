...
- Marv installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Marv"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Marv and restart the gateway.
5. Send a first DM and approve pairing (`marv pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Marv"` then scan the QR in Signal.
...
4. Configure Marv, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `marv pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Marv at it:
...
This skips auto-spawn and the startup wait inside Marv. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  ...
  - `marv pairing approve signal <CODE>`
...
- **Typing indicators**: Marv sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Marv forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `marv doctor --fix`.
...
- `agents.defaults.groupChat.mentionPatterns` (Signal does not support native mentions).
