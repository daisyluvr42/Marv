---
summary: "Mattermost bot setup and Marv config"
read_when:
...
```bash
marv plugins install @openclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
marv plugins install ./extensions/mattermost
```
...
4. Configure Marv and start the gateway.
...
- Approve via:
  - `marv pairing list mattermost`
  - `marv pairing approve mattermost <CODE>`
...
Use these target formats with `marv message send` or cron/webhooks:
