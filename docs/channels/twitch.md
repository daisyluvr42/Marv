...
read_when:
  - Setting up Twitch chat integration for Marv
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Marv connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
marv plugins install @marv/twitch
```

Local checkout (when running from a git repo):

```bash
marv plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MARV_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "marv", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MARV_TWITCH_ACCESS_TOKEN env var)
...
```bash
MARV_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
