...
```bash
marv plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Marv will offer the local install path automatically.
...
3. Configure Marv with those credentials.
...
- Talk to Marv via Teams DMs, group chats, or channels.
...
- On startup, Marv resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring Marv, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "Marv in Teams", "full": "Marv in Teams" },
...
Marv handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Marv only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
```bash
marv message send --channel msteams \
...
marv message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
marv message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
marv message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
marv message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
