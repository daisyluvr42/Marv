...
Tlon is a decentralized messenger built on Urbit. Marv connects to your Urbit ship and can
...
```bash
marv plugins install @openclaw/tlon
```

Local checkout (when running from a git repo):

```bash
marv plugins install ./extensions/tlon
```
...
By default, Marv blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `marv message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Marv replies in-thread.
