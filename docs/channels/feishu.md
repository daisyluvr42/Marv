...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Marv to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
marv plugins install ./extensions/feishu
```
...
If you just installed Marv, run the wizard:
...
2. Configuring app credentials in Marv
...
1. You already ran `marv channels add` for Feishu
2. The gateway is running (`marv gateway status`)
...
```bash
marv pairing approve feishu <CODE>
```
...
2. Run `marv logs --follow` and look for `chat_id`
...
2. Run `marv logs --follow` and look for `open_id`
...
| Command                | Description                   |
| ---------------------- | ----------------------------- |
| `marv gateway status`  | Show gateway status           |
| `marv gateway install` | Install/start gateway service |
| `marv gateway stop`    | Stop gateway service          |
| `marv gateway restart` | Restart gateway service       |
...
5. Ensure the gateway is running: `marv gateway status`
6. Check logs: `marv logs --follow`
