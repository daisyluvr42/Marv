...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Marv
...
Running Marv via Node with `tsx` fails at startup with:

```
[marv] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `marv status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch marv.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node marv.mjs status` works on Node 25.
