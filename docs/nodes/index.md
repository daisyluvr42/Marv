...
macOS can also run in **node mode**: the menubar app connects to the Gateway’s WS server and exposes its local canvas/camera commands as a node (so `marv nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `marv nodes pending/approve/reject`) is a separate gateway-owned
...
```bash
# Terminal A (keep running): forward local 4243 -> gateway 127.0.0.1:4242
...
marv nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
Marv can run a **headless node host** (no UI) that connects to the Gateway
...
- The macOS menubar app connects to the Gateway WS server as a node (so `marv nodes …` works against this Mac).
