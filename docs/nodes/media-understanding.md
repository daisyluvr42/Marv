...
Marv can **summarize inbound media** (image/audio/video) before the reply pipeline runs. It auto‑detects when local tools or provider keys are available, and can be disabled or customized. If understanding is off, models still receive the original files/URLs as usual.
...
- If `<capability>.enabled: true` but no models are configured, Marv tries the
...
## Provider support matrix (Marv integrations)
