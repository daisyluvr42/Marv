...
read_when:
  - You want to place an outbound voice call from Marv
...
Voice calls for Marv via a plugin. Supports outbound notifications and
...
- Use `marv voicecall ...` or the `voice_call` tool
...
```bash
marv plugins install ./extensions/voice-call
