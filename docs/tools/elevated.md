...
- **Availability gates**: `tools.elevated` is the global baseline. `agents.defaults.tools.elevated` can further restrict elevated per agent (both must allow).
...
- Durable agent gate: `agents.defaults.tools.elevated.enabled` (optional; can only further restrict).
- Durable agent allowlist: `agents.defaults.tools.elevated.allowFrom` (optional; when set, the sender must match **both** global + durable-agent allowlists).
