...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `marv sandbox explain`.
...
Legacy `agent.*` configs are migrated by `marv doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=marv-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MARV_STATE_DIR:-$HOME/.marv}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
