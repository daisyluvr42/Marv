...
- `commands.config` (default `false`) enables `/config` (reads/writes `marv.json`).
...
- `/approve <requestId> allow-once|allow-always|deny` (resolve exec approval prompts; `taskId` also works when unique)
...
- For full provider usage breakdown, use `marv status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Marv session logs.
...
```
/debug show
/debug set messages.responsePrefix="[marv]"
...
- Overrides apply immediately to new config reads, but do **not** write to `marv.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`marv.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[marv]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/marv`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
