---
summary: "Chrome extension: let Marv drive your existing Chrome tab"
read_when:
...
The Marv Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate marv-managed Chrome profile.
...
Marv then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Marv release (npm package) as static files. There is no separate “build” step.

After upgrading Marv:

- Re-run `marv browser extension install` to refresh the installed files under your Marv state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Marv ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Marv to control.
...
- `ON`: attached; Marv can drive that tab.
...
Debugging: `marv sandbox explain`
...
`marv browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `marv browser extension install` first to copy the extension to a stable location under your Marv state directory.
...
- **This is not isolated** like the dedicated marv-managed profile.
