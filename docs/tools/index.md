...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `marv.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Marv logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:marv`: all built-in Marv tools (excludes provider plugins)
...
Marv tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `marv configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Marv-managed browser.
...
- Quick smoke: `marv nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `marv gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Marv clamps `tools.sessions.visibility` to `tree`.
