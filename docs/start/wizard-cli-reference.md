...
read_when:
  - You need detailed behavior for marv onboard
...
This page is the full reference for `marv onboard`.
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `marv pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `marv health`.
    - `marv status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
