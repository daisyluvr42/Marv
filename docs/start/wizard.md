...
The onboarding wizard is the **recommended** way to set up Marv on macOS,
...
`marv dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `marv configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.marv/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `marv doctor` first.
</Note>
...
- CLI command reference: [`marv onboard`](/cli/onboard)
