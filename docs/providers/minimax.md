---
summary: "Use MiniMax M2.1 in Marv"
read_when:
  - You want MiniMax models in Marv
...
```bash
marv plugins enable minimax-portal-auth  # skip if already loaded.
marv gateway restart  # restart if gateway is already running
marv onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](extensions/minimax-portal-auth) for details.
...
Configure manually via `marv.json`:
...
- Use `marv models list` and `marv models set minimax/MiniMax-M2.1` to switch.
...
- Running `marv configure` and selecting **MiniMax M2.1**, or
