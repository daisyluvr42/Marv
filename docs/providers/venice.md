---
summary: "Use Venice AI privacy-focused models in Marv"
read_when:
  - You want privacy-focused inference in Marv
...
```bash
marv onboard --auth-choice venice-api-key
```
...
```bash
marv chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Marv shows all available Venice models. Pick based on your needs:
...
```bash
marv models set venice/claude-opus-45
marv models set venice/llama-3.3-70b
```

List all available models:

```bash
marv models list | grep venice
```

## Configure via `marv configure`

1. Run `marv configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Marv automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
marv chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
marv chat --model venice/claude-opus-45

# Use uncensored model
marv chat --model venice/venice-uncensored

# Use vision model with image
marv chat --model venice/qwen3-vl-235b-a22b

# Use coding model
marv chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `marv models list` to see currently available models. Some models may be temporarily offline.
