...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("marv-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("marv-bin-sherpa-");
      const modelDir = await makeTempDir("marv-sherpa-model-");
      ...
      const cfg: MarvConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("marv-bin-whispercpp-");
      const modelDir = await makeTempDir("marv-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("marv-bin-gemini-");
      ...
      const cfg: MarvConfig = { tools: { media: { image: {} } } };
