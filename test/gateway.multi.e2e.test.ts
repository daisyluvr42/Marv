...
import { connectGatewayClient } from "../src/core/gateway/test-helpers.e2e.js";
...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `marv-e2e-${name}-`));
  const configDir = path.join(homeDir, ".marv");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MARV_SKIP_BROWSER_CONTROL_SERVER: "1",
