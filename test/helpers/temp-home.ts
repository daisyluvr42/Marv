...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    marvHome: process.env.MARV_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MARV_HOME", snapshot.marvHome);
  restoreKey("MARV_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MARV_HOME.
  delete process.env.MARV_HOME;
  process.env.MARV_STATE_DIR = path.join(base, ".marv");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "marv-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".marv", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
