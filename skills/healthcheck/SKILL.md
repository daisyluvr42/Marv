---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Marv deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Marv cron scheduling for periodic checks, or version status checks on a machine running Marv (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Marv, then align it to a user-defined risk tolerance without breaking access. Use Marv security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Marv changes the host firewall, SSH, or OS updates; it does not.
...
5. Marv gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Marv can access sensitive data.
...
### 2) Run Marv security audits (read-only)

As part of the default read-only checks, run `marv security audit --deep`. Only offer alternatives if the user requests them:

1. `marv security audit` (faster, non-probing)
2. `marv security audit --json` (structured output)

Offer to apply Marv safe defaults (numbered):

1. `marv security audit --fix`

Be explicit that `--fix` only tightens Marv defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Marv version/update status (read-only)

As part of the default read-only checks, run `marv update status`.
...
- Credential hygiene notes (location of Marv creds, prefer disk encryption)
...
After Marv install or first hardening pass, run at least one baseline audit and version check:

- `marv security audit`
- `marv security audit --deep`
- `marv update status`

Ongoing monitoring is recommended. Use the Marv cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `marv cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `marv update status`
...
Before creating, `marv cron list` and match on exact `name`. If found, `marv cron edit <id> ...`.
If not found, `marv cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `marv update status` (preferred for source checkouts and channels)
2. `npm view marv version` (published npm version)

## Marv command accuracy

Use only supported commands and flags:

- `marv security audit [--deep] [--fix] [--json]`
- `marv status` / `marv status --deep`
- `marv health --json`
- `marv update status`
- `marv cron add|list|runs|run`

Do not invent CLI flags or imply Marv enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Marv compaction:
